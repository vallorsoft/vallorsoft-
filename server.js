...
async function sendInviteEmail(toEmail, kod, pozicio, cegNev, igazgatoNev) {
  ...
  var registerUrl = process.env.APP_URL || 'https://vallorsoft.onrender.com';
  var udvozles = igazgatoNev ? ('Tisztelt ' + igazgatoNev + '!') : 'Tisztelt Partnerunk!';
  var cegSor = cegNev ? ('A <b style="color:#fff;">' + cegNev + '</b> ceg szamara') : 'Az On szamara';
  ...
  var htmlBody =
    '<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#05070b;color:#e9eef5;padding:32px;border-radius:16px;">' +
      ...
      '<p style="color:#8a97a8;margin-bottom:16px;">A <b style="color:#fff;">VallorSoft</b> cegtol kapta ezt az emailt.</p>' +
      '<p style="color:#8a97a8;margin-bottom:24px;">' + cegSor + ' aktivalva lett a hozzaferes <b style="color:#fff;">' + pozicio + '</b> szerepkorben.</p>' +
      '<div style="background:#141c25;border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:20px;margin-bottom:24px;text-align:center;">' +
        '<div style="font-size:12px;color:#8a97a8;margin-bottom:8px;text-transform:uppercase;letter-spacing:1px;">Meghivokodja</div>' +
      ...
      '<div style="background:#141c25;border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:20px;margin-bottom:24px;">' +
        '<div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:12px;">Regisztracio lepesei:</div>' +
        '<ol style="color:#8a97a8;font-size:13px;line-height:1.8;padding-left:20px;margin:0;">' +
          ...
          '<li>Adja meg a nevet, email cimet es egy jelszot</li>' +
          '<li>A meghivokod mezobe irja be: <b style="color:#fff;">' + kod + '</b></li>' +
          '<li>Kattintson a <b style="color:#fff;">Regisztracio</b> gombra</li>' +
          '<li>Ezutan bejelentkezhet a <a href="' + registerUrl + '/login" style="color:#3b82f6;">' + registerUrl + '/login</a> oldalon</li>' +
        '</ol>' +
      '</div>' +
      '<p style="font-size:11px;color:#8a97a8;margin:0;">Ez az email automatikusan lett elkuldve a VallorSoft rendszer altal.</p>' +
    '</div>';
  ...
  try {
    var resp = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [toEmail],
        subject: 'VallorSoft - Meghivo (' + (cegNev || 'VallorSoft') + ')',
        html: htmlBody
      })
    });
    ...
    console.error('Resend hiba:', err.message);
  }
}
