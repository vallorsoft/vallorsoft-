...
1. **Migráció `db/driver-payment-item-alloc.sql`** (ÚJ, idempotens): `driver_payment_group_items.alloc_ron NUMERIC(12,2)` (NULL = teljes tétel — a régi „egész tétel" viselkedés; szám = ennyi RON-egyenérték erre a tételre). Az `UNIQUE (earning_id)` megkötés ELEJTVE → egy tétel több részletben (több csoportból) is kifizethető; a túl-allokálást a szerver őrzi.
2. **Szerver — vezetett allokáció** (`handlers/fleetCompliance.js`): `earningPaymentGroupCreate` új opcionális `allocations[{earning_id, alloc_ron}]` bemenet + túl-allokálás-ellenőrzés (meglévő + új ≤ a tétel RON-igénye; és a teljes allokáció ≤ a kifizetés RON-ja); a legacy multi-select út VÁLTOZATLAN (alloc_ron NULL). `computeDriverAllocation` mostantól a tárolt `alloc_ron`-t összegzi (NULL=teljes), a részleges csoportok fel-nem-osztott maradékát a FIFO-poolba adja. Új **`getDriverEarningAllocation(email)`** handler: a kifizetetlen (vagy részlegesen fizetett) járandóságok HÓNAPRA bontva (earning_date, legrégebbi elöl), per-tétel hátralékkal (RON + saját valuta) + effektív BNR — a vezetett modál adatforrása.
3. **Szerver — láthatóság**: `earningList` a TELJESEN kifizetett tételt kihagyja, a RÉSZLEGESEN fizetettet MEGTARTJA `settled_ron`/`remaining_ron`/`remaining_cur`/`partial` mezőkkel. `getMonthlySettlementSheet` minden kifizetéshez `covers[]` (melyik járandóságot/hónapot fedezte, `alloc_ron`-nal); `earningPaymentGroupGet` items `alloc_ron`-t is ad. Mind best-effort/migráció-tudatos.
4. **Kliens — vezetett kifizetés-modál** (`public/fleet-extra-v2.js`): a 💵 Részleges / ✅ Teljes gomb (és a sor-szintű 💰) a MEGLÉVŐ sima kifizetés helyett a **`#dcAllocModal` vezetett kártyás modált** nyitja: kifizetés-kártya (összeg/valuta/mód/dátum/BNR) + „🔄 Auto (legrégebbitől)" FIFO-előtöltés + hónap-kártyák a tételekkel (pipa + hátralék + „ebből fizetek" mező a saját valutában), élő „−X / marad Y" per tétel + összegzés (Kifizetés / Allokálva / Nem allokált–Túllépés). Mentés → `earningPaymentGroupCreate` allocations-szel; ha nincs kifizetetlen járandóság → a sima (előleg) kifizetés nyílik.
5. **Kliens — „mit fedez" a dokumentumokon**: Decont lunar + Kifizetés-történet (payment-sor alatt „↳ Fedezi: 2026. 08.: Diurna, …"), Csoportos bizonylat + on-screen csoport-részlet (per-tétel „Fizetve: X RON"), járandóság-lista (részleges tétel: „Fizetve X / marad Y" + halvány borostyán sor).
6. **CSS** — új `#dcAllocModal` blokk: VILÁGOS, kártyás, kék/fehér/fekete kontraszt (mint a fuvar-kiírás), reszponzív ≤640px. **i18n** 17 új `fe.al.*` kulcs (RO-alap + HU). Cache-bust `?v=20260918payalloc`.
7. **Teszt** — `earningPaymentGroupCreate` guided + túl-allokálás (payment-groups: +2); `getDriverEarningAllocation` (driver-earnings-payments: +3); `earningList` részleges/teljes annotáció (2 átírva). Valós DOM (jsdom) smoke a vezetett modálra (FIFO-előtöltés, részleges levonás, submit-payload). **1264 Jest zöld**, nincs regresszió.
...
1. **Új „📇 Dokumentum-nyilvántartás" menüpont** az admin+manager konzol **Dokumentumok** csoportjában (`doc-register`, `feature-catalog.js` → developer cégenként ki/be kapcsolhatja). Mappa-alapú (folder), minden mappának SAJÁT automatikus sorszámozása (pl. `FCT-2026-0001`).
2. **Migráció `db/document-register.sql`** (ÚJ, idempotens): `doc_register_groups` (mappák: név + prefix + `year_reset` évente-újrakezd + `pad` számjegy + szín + megjegyzés) · `doc_register_counters` (cég+csoport+év atomi számláló — `year=0` a folytonos számozáshoz, `ON CONFLICT DO UPDATE` mint a fuvar-szám) · `doc_register_entries` (kiadott sorszámok: reg_no + dátum + cím + partner + összeg + státusz `with_doc`|`reserved`|`void`) · `doc_register_files` (csatolt dokumentumok base64 data URL-ként, mint az `order_documents`). Mind `company_id`-vel horgonyzott.
3. **Handler `handlers/documentRegister.js`** (ÚJ, `routes/execute.js`-be regisztrálva) — **12 RPC**, mind Admin/Manager, `company_id`-szűrt, paraméteres SQL, audit: `docRegGroupList/Save/Delete` (mappák + statisztika: bejegyzés-szám, dokumentumos/foglalt darab, utolsó szám) · `docRegEntryList` (mappán belül, szűrők: keresés/dátum/státusz) · `docRegEntryCreate` (tranzakcióban atomi sorszám-kiadás; dokumentummal → `with_doc`, nélküle → `reserved` foglalt; opcionális kézi `manual_no` egyediség-ellenőrzéssel; csoport-tulajdon ellenőrzés a kapcsolat-foglalás ELŐTT) · `docRegEntryUpdate`/`Delete` (a sorszám sosem változik/adódik újra) · `docRegEntryAddFiles`/`docRegEntryFiles`/`docRegFileGet`/`docRegFileDelete` (fájl hozzáadás/lista/letöltés/törlés — 0 fájlnál a bejegyzés visszaáll foglaltra) · `docRegSearch` (globális keresés a mappák között reg_no/cím/partner/megjegyzés szerint).
4. **Kliens `public/document-register.js`** (ÚJ, `DocRegister.mount('docRegisterBox')`): kétnézetes — **mappák rácsa** (kártyák szín-chippel, prefix-badge, összes/📎 dokumentumos/🔖 foglalt számláló, utolsó szám; szerkesztés/törlés; mappa-szűrő) + fent **globális kereső** (élő, 2+ karakter, találat a mappa nevével → kattintásra a mappa nyílik). Egy mappát megnyitva **bejegyzés-lista**: KPI-sáv + szűrő (keresés/dátum-tartomány/státusz) + tábla (szám/dátum/cím/partner/összeg/státusz-pilula/📎fájlok/műveletek). **Új bejegyzés** modal (dátum-alap ma, cím/partner/összeg+pénznem/megjegyzés, opcionális kézi szám, opcionális fájl-feltöltés multi) · **Sorszám foglalása** (dokumentum nélküli szám) · **Fájlok** modal (letöltés data URL-ből, hozzáadás, törlés).
5. **i18n** ~48 új `dr.*` + `nav.docRegister` + `common.deletedOk` kulcs (RO-alap + HU). Bekötés: `admin.html`/`manager.html` sub-tab + pane + script-include; `admin.js`/`manager.js` `loadTab`. Cache-bust `?v=20260917docreg`.
6. **Teszt** — új `tests/integration/document-register.test.js` (+18 eset: szerep-kapuk, tenant-szűrés, mappa-validáció + duplikátum, bejegyzés-lista szűrő-fehérlista, sorszám-kiadás tranzakció — foglalt vs. dokumentumos, kézi-szám ütközés ROLLBACK, idegen csoport, globális keresés, fájl-letöltés tenant). **1253 Jest zöld** (1234 → 1253), require-sweep tisztán betölti az új modult, nincs regresszió.
...
1. **Gyökér (cross-currency semantic bug):** a `getDriverBalance` (`handlers/fleetCompliance.js`) valuta-specifikusan vonta le a kifizetést a járandóságból: `bal_eur = earned.EUR − paid.EUR`; `bal_ron = earned.RON − paid.RON`. Ha az egyik valutában TÚLFIZETÉS, a másikban meg TARTOZÁS volt (közös eset RON-készpénzes kifizetéskor EUR-járandósággal), a felesleg NEM íródott le a másikra → a sofőrnek látszólag még sok EUR járt.
2. **Fix (`handlers/fleetCompliance.js`):** cross-currency settlement a mai BNR-en. Ha `bal_eur > 0 && bal_ron < 0` → RON-túlfizetést EUR-ra váltjuk (`applied = min(bal_eur, |bal_ron|/bnr)`); `bal_eur -= applied`, `bal_ron += applied*bnr`. Fordított irány ugyanígy. A `balance.eur`/`balance.ron` mostantól a **beszámolt (végső)** érték; új `balance.eur_raw`/`ron_raw` a beszámolás előtti nyers érték + `balance.cross_ron_to_eur`/`cross_eur_to_ron` audit-mezők (a UI ezt magyarázza el a sofőrnek). Peto-eset: 892 RON / 5.05 = 176.63 EUR levonás → maradt 333.37 EUR + 0 RON.
3. **Kliens (`public/fleet-extra-v2.js` `_dcBalanceCard`)** — a 6 csempés egyenleg-sáv alá új „🔁 Cross-valuta beszámítás" magyarázó sor, csak ha ténylegesen történt átváltás. Formátum: „RON-túlfizetés levonva EUR-jár.-ból — 892,00 RON → 176,63 EUR (BNR-en 1 EUR = 5,0500 RON; nyers: 510,00 EUR / -892,00 RON)". A meglévő payment-modal (`dcOpenPayment`) alapból a beszámolt egyenleget preloadolja → a „✅ Teljes kifizetés" gomb ezúttal a valóban fizetendő értéket ajánlja.
4. **Új: 🧾 Kifizetés-történet nyomtatás** — új gomb az egyenleg-akció-sávban a 💵/✅ mellett. `dcOpenPayHistory` a MEGLÉVŐ Decont lunar (📄) modalt nyitja új `_dcSheetMode = 'payments-only'` móddal, rolling 12M preset alapból. A `_dcRenderSheetHtml` payments-only módban skippeli a járandóság-táblát + egyenleg-blokkot, más badge-szöveget renderel (zöld gradiens „ISTORIC PLĂȚI" / „KIFIZETÉS-TÖRTÉNET"). Ugyanaz a hivatalos cég-fejléc (logó + adresa/CUI/tel + doc-badge) + sofőr-fejléc + kifizetés-tábla (dátum · módszer · összeg · BNR · RON-egyenérték) + kifizetve total + aláíró blokk (sofőr / cég + pecsét). Print/e-mail címzést a mód szerint állítja (`fe.ph.docTitle`/`fe.ph.emailSubject`). Nincs új szerver-handler — a meglévő `getMonthlySettlementSheet` amúgy is max 2 éves tartományt engedélyez.
5. **i18n** — 9 új kulcs (`fe.dc.crossTitle`/`crossRonToEur`/`crossEurToRon`/`crossAt`/`crossRaw` + `fe.ph.openBtn`/`openTitle`/`docTitle`/`emailSubject`, RO-alap + HU). Cache-bust `?v=20260910crossph` (admin.html + manager.html — i18n.js + fleet-extra-v2.js).
6. **Teszt** (`tests/integration/driver-earnings-payments.test.js`) — +3 új eset a `getDriverBalance` describe-jébe: (a) Peto-eset EUR-tartozás + RON-túlfizetés (500 → 333.37 EUR, 0 RON, cross_ron_to_eur=176.63); (b) fordított irány RON-tartozás + EUR-túlfizetés; (c) BNR-hiba → nincs beszámítás, raw = settled. **1136 Jest zöld** (1133 → 1136, +3 net). Nincs regresszió — a régi „EUR járandóság + részleges EUR kifizetés" eset változatlan (nincs cross-scenario).
...
1. **Gyökér (aláírás vs. pecsét):** a régi `stamps` táblában egyetlen `base64_png` oszlop volt, ezért a `saveAdminSigDraw` (kanavász → PNG) és a `saveAdminSigFile` (feltöltött PNG) EGYMÁST FELÜLÍRTA — a user csak vagy-vagy tarthatta.
2. **Migráció `db/signature-stamp-workspace.sql`** (idempotens, auto-fut) — `stamps.signature_base64` + `stamps.stamp_base64` új oszlopok (a régi `base64_png` megmarad legacy kompatibilitásért: a sofőr menetlevél PDF ráégetés a `routes/soferApi.js` `/api/pdf-download`-ban abból dolgozik); egyszeri backfill: a meglévő `base64_png` átmásolódik a `stamp_base64`-ba (pecsétnek tekintjük, ez a régi ráégetési viselkedés). Új `pdf_workspace_docs` tábla (id/user_email/company_id/file_name/original_base64/signed_base64/file_size/created_at) a közvetlen munkatérhez, index `user_email + created_at` és `created_at` szerint.
3. **Új handlerek (`handlers/documents.js`):** `sigAssetsGet` (mind a szeparált aláírás + pecsét — legacy `base64_png` pecsét-fallbackkel a backfill előtti sorokra) és `sigAssetsSave({signature?, stamp?})` — hasOwnProperty alapú részleges update, csak a MEGADOTT mezőket írja; a pecsét mentése a legacy `base64_png`-t is szinkronban tartja, hogy a menetlevél PDF változatlan képet kapjon. Data URL 1.5 MB méret-korlát.
4. **Új handler-modul `handlers/pdfWorkspace.js`** (regisztrálva `routes/execute.js`) — 5 új RPC (Admin/Manager, cégre + user_email-re szűrt, cross-tenant védelem, audit): `pdfWorkspaceUpload` (max 15 MB, `application/pdf` mime-fehérlista, filename sanitizálás, `genDocId('WSP')`), `pdfWorkspaceList` (csak a saját user 24h ablakos sorai, max 100), `pdfWorkspaceGet(id, which)` (original|signed), `pdfWorkspaceSaveSigned(id, base64)` (aláírt verzió mentése), `pdfWorkspaceDelete(id)`. A Sofer szerep nem éri el.
5. **Új scheduler `services/scheduler.js` `startPdfWorkspaceCleanup`** — 6 óránként törli a >24h régebbi `pdf_workspace_docs` sorokat; migráció-hiány (`42P01`) → csendes skip; bind `server.js`.
6. **Admin/Manager `data-pane="signature"` átalakítás** (`public/admin.html` + `manager.html`): (a) rajzoló kártya alá aláírás-előnézet doboz (`#adminSigOnly`), gomb címke „Aláírás mentése" (nem „Mentés"); (b) a bélyegző-előnézet doboz átkerült a bélyegző kártya belsejébe (szimmetrikus); (c) új alsó kártya „📄 Dokumentum aláírása / pecsételése" — PDF-feltöltő + munkatér-lista táblázatban (fájlnév/méret/állapot/hátralévő idő 24h-ig/műveletek: ✍️ Aláírás/Pecsét · ⬇️ Eredeti/Aláírt · 🗑 Törlés).
7. **signModal bővítés** (`admin.html` + `manager.html` + `console-shared.js`): új „2b. Mentett aláírás" blokk a stamp mellett — a Beállításban rajzolt aláírás egy kattintással a PDF-re helyezhető (nem csak a modal-canvas inline rajz). Az `openSignModal` egy hívással (`sigAssetsGet`) tölti be MINDKÉT képet, `_signMode='order'|'workspace'` állapot, hogy a `burnAndSaveDoc` a megfelelő végpontra mentsen (`orderDocSaveSigned` vs. `pdfWorkspaceSaveSigned`). Új `openPdfWorkspaceSign(id)` — a signModal-t újrahasználva a workspace-dokumentumon dolgozik (a PDF-tartalmat közvetlenül a `pdfWorkspaceGet`-ből tölti, nem az order-doc útról).
8. **Kliens-oldali új közös függvények** (`console-shared.js`): `addSavedSigToPage`, `pdfWsUpload/pdfWsDownload/pdfWsDelete/loadPdfWorkspaceList/openPdfWorkspaceSign`. A `saveAdminSigDraw`/`saveAdminSigFile` átállítva `sigAssetsSave`-re (a régi `stampSave` legacy — a menetlevél PDF ráégetéséhez marad meg). `loadAdminSigPreview` mindkét dobozt frissíti.
9. **`admin.js` + `manager.js`** — `loadTab('signature')` mostantól hívja a `loadPdfWorkspaceList`-et is (init).
10. **i18n** — 22 új kulcs (`sig.saveSig`/`sig.noSig`/`sig.ws*` + `sg.step2b`/`sg.noSavedSig`/`sg.addSavedSig` + `cs.pickPngFirst`/`pickPdfFirst`/`pdfOnly`/`fileTooLarge`/`uploaded`/`deleted`/`loading`/`noSavedSig`), RO-alap+HU. Cache-bust `?v=20260908pgletter → ?v=20260909sigws` (admin/manager html — i18n.js + console-shared.js + admin.js + manager.js).
11. **Teszt** — új `tests/integration/pdf-workspace.test.js` (+27 eset: `sigAssetsGet`/`sigAssetsSave` szeparált mezők + legacy fallback + EGYIDEJŰ mentés + méret-korlát; `pdfWorkspaceUpload/List/Get/SaveSigned/Delete` szerep-kapu + cross-tenant WHERE + 24h ablak + PDF mime + méret-korlát + nyers b64 → data URL). **1160 Jest zöld** (1133 → 1160), a régi `documents.test.js` legacy stamp-tesztek változatlanul zöldek.
...
1. **`db/driver-receipt-scans.sql`** (ÚJ, idempotens): `driver_receipt_scans` tábla — kind (fuel/purchase/other), AI-kivont mezők JSONB-ben, opc. 128px thumbnail (nem a teljes kép — adat-minimalizálás), status (pending / attached / deleted), waybill_id ha attached, teljes életciklus-időbélyegek. Multi-tenant + indexelt.
2. **`handlers/receiptScan.js`** — a `scanReceipt` a sikeres AI-kiolvasás után best-effort INSERT-el egy `status='pending'` sort, és a `pending_id`-t visszaadja a kliensnek. Ha a tábla nem létezik (migráció nem futott le), a scan-válasz akkor is `ok=true` — csak a nyilvántartás marad ki. Új RPC-k: `listPendingReceipts` (Sofer csak sajátot, session-emailre kényszerítve; Admin/Manager email-paraméterrel a cégen belül), `attachPendingReceipt(id, waybill_id?)` (pending → attached; Sofer csak sajátot email-gate-tel a WHERE-ben, Admin/Manager cégen belül bármelyiket), `deletePendingReceipt(id)` (soft-delete pending-ből; már-attached sort NEM töröl — állapot-invariáns).
3. **`handlers/driverActivity.js`** — új sub-query a `driver_receipt_scans` táblára: a pending sorok `fuel_pending` / `purchase_pending` esemény-típusként kerülnek a timeline-ba (☁️ ikon + jelölés a fejlécben), az attached-ek normál `fuel` / `purchase` típussal + „📄 menetlevélben" badge-dzsel. A counts is bővítve (`fuel_pending`, `purchase_pending`) — a fejléc-számláló-chip azonnal jelzi az adminnak, hány pending van. Best-effort try/catch (régi cég-DB migráció-nélkül → üres lista).
4. **`public/sofer.js`** — `_scanReceiptTry` a 128px thumbnailt is elküldi (`thumb_b64` payload-mezőben, max ~150 KB — a teljes kép SOSEM megy DB-be); a válaszban kapott `pending_id`-t a localStorage queue-tétel mellé menti. `rrAccept` → best-effort `attachPendingReceipt` (pending → attached). `rrRemove` (a Discard hívja) → best-effort `deletePendingReceipt` (pending → deleted). Mindkettő `keepalive:true` — a szerver-oldali állapot mindig szinkronban a kliens-oldalival.
5. **`public/driver-activity.js`** — a timeline-esemény cím mögé színes badge kerül a scan-eseményekhez: `☁️ Csak felhőben (menetlevélben még nincs)` (borostyán) vagy `📄 Menetlevélben` (zöld). A fejléc-chip-sáv 2 új chip-je (`fuel_pending`, `purchase_pending`) narancs tónusú, ha van pending — vizuálisan azonnal látszik, ha egy sofőrnek „lebegő" bonjai vannak.
6. **CSS + i18n** — `.da-scan-pending` / `.da-scan-attached` badge + `.da-cnt-warn` chip-tónus + pending idővonal ikon-kör sárga akcens (világos + sötét téma). 5 új `da.*` + `nav`-mentes i18n kulcs (RO-alap + HU).
7. **Adatvédelem** — a THUMBnail is opcionális (NULL-ozható); a teljes kép soha nem kerül DB-be (memóriában csak a Gemini-hívás alatt él). Cross-tenant védelem: minden SQL `company_id`-szűrt, Sofer email-gate a WHERE-ben. Audit: `receipt.scan` / `receipt.attach` / `receipt.delete` (metaadat, semmi kép).
8. **Teszt** — `tests/integration/driver-receipt-scans.test.js` (+10 új eset: scanReceipt INSERT sikeres + DB-hiba tolerancia; attach Sofer email-gate + Admin bypass + 0-sor kezelés; delete Sofer soft-delete + already-attached NEM törölhető + érvénytelen id; list Sofer sajátra kényszerítés + Admin email-param + status fehérlista). **1031 Jest zöld** (1021 → 1031, +10 új). Cache-bust `?v=20260901scan` (admin/manager/sofer: style.css + i18n.js + driver-activity.js + sofer.js).
...
1. **Új menüpont** — **Flotta → 🎬 Sofőr-aktivitás** (`sofer-activity`, admin + manager, `feature-catalog.js`-be új kulcs, alapból BE). A régi entitás-adatlap-modal (fuvar-részletek) érintetlen marad — ez egy külön áttekintő nézet.
2. **Backend** — új `handlers/driverActivity.js` (`routes/execute.js`-be regisztrálva): **`getActivityDrivers(from,to)`** a sofőr-listához (avatar-hoz név/email, KPI-k: fuvarok, menetlevelek, km, feltöltött fotók, utolsó aktivitás dátum — Admin/Manager, cégre szűrt, három közös aggregátor lekérdezéssel); **`getDriverActivity(email, orderId?, from, to)`** kronológikus timeline egy sofőrre (opcionális egyetlen fuvarra szűkítve): milestone-ok (`sosit_incarcare_at`/`incarcat_at`/`sosit_descarcare_at`/`descarcat_at` a fuvar-időbélyegekből), menetlevél-beküldések + bennük tankolás + vásárlás sorok külön eseményként a saját dátumukon (`fuvarlevelek.alimentari`/`achizitii`), UIT-kódok (`order_uit_codes` a fuvar-listára szűrve), sofőr által feltöltött fotók (`documents` `email_sofer`-re + `company_id`-re szűrve, thumb + full URL a meglévő `/api/doc-download`-ra), határátlépések (`border_crossings` sofőr-emailre szűrve), bug-jelzések (`bug_reports`). **Cross-tenant védelem:** a sofőr `users.company_id = req.session.user.company_id` a válasz előtt (idegen cég sofőrje → 0 sor → „Soferul nu a fost gasit"). Best-effort try/catch az opcionális táblákra — séma-eltérés esetén üres tömb, a fő eredmény nem hasal el.
3. **Frontend** — új `public/driver-activity.js` (~380 sor, `window.DriverActivity` IIFE): **2-oszlopos kártyás layout** — BAL: sofőr-kártya lista keresővel + 4 KPI-vel + monogram-avatarral (determinisztikus hash-szín) + „🕐 utolsó aktivitás" relatív időbélyeggel (ma / tegnap / N napja); JOBB: kiválasztott sofőr fejléc (nagy avatar + kontakt + fuvar-választó legördülő + 8 db esemény-típus szűrő chip élő számlálóval, kattintva szűr), fotó-galéria grid (thumbnail-hover-emeléssel), **kronológikus timeline** függőleges idővonal-oszloppal + színes ikon-körökkel típusonként (📍 milestone-kék, 📄 menetlevél-indigó, ⛽ tankolás-borostyán, 🛒 vásárlás-rózsa, 📷 fotó-zöld, 🛣️ UIT-világoskék, 🛂 határ-narancs, 🐛 bug-piros, ⛔ áru-leadás-narancs), fuvar-tag pilulával ha releváns.
4. **Fotó-lightbox** — külön modal (`.modal-back` + `.modal`, projekt mintát követve): teljes-képernyős kép, ← → gombok a fotók között, „Megnyit új tabon" a `/api/doc-download`-ra, ESC / háttér-kattintás bezár.
5. **Időszak-kezelés** — fejlécen dátum-tartomány + gyors preset (📅 e hó / 📆 e hét); default: aktuális hó 1-e → hónap vége. A dátum-változás mindkét oszlopot újratölti.
6. **Kontrasztos + kártyás CSS** (`public/style.css` végén ~200 új sor, kizárólag `#driverActivityBox` + `#daLightbox` scope-ban): világos + sötét téma, kártyák 1.5px keret + shadow-lépcső, aktív sofőr-kártya indigó keret + halo, KPI-rács 4 oszlopban (mobilon 2), timeline függőleges vonal + ikon-körök hátszínezettek típusonként, fotó-grid `auto-fill(140px)`, reszponzív (≤960px → 1 oszlop, ≤640px → KPI 2×2 rács).
7. **i18n** ~30 új `da.*` + `nav.driverActivity` kulcs (RO-alap + HU). Cache-bust `?v=20260901daact` (admin + manager: style.css + i18n.js + új driver-activity.js).
8. **Nincs új tábla, nincs séma-változás** — pure read-only aggregátor a meglévő táblákból.
9. **Teszt** — `tests/integration/driver-activity.test.js` (+10 új eset: szerep-kapu Sofer-tiltás, `getActivityDrivers` üres cég + tartomány-átadás + KPI összefésülés 3 aggregátorból + company_id-szűrt Sofer WHERE; `getDriverActivity` üres email + cross-tenant védelem + teljes timeline 8 forrásból 12 eseménnyel DESC rendezve + opcionális táblák hibája → ok:true üres tömbökkel). **1020 Jest zöld** (1010 → 1020, +10), require-sweep 137 modul 0 hiba.

---

## 2026-09-01 — ÚJ: Sofőr-elszámolás 2.0 — járandóság + kifizetés (EUR/RON + BNR) kártyás felületen

**Gyökér:** a Sofőr-elszámolás (decont) eddig csak a menetlevél-alapú kassza-egyenleget (kiadott előlegek + készpénzes költések + diurna napok) mutatta. Nem lehetett kézzel beírni bónuszt / prémiumot / egyedi diurnát („3× 10 EUR bónusz + 6 nap × 70 EUR/nap diurna"), nem lehetett külön nyilvántartani a járandóságot és a tényleges kifizetést, és nem lehetett egy kattintással részlegesen vagy teljesen kifizetni. Kellett EUR/RON választás is, és hogy a BNR-árfolyam a **kifizetés pillanatában** mentődjön a rekordba.

1. **`db/driver-earnings-payments.sql`** (ÚJ, idempotens): két új tábla — `driver_earnings` (amivel a cég TARTOZIK a sofőrnek: kind + label + quantity × unit_amount + currency, EUR|RON, `total_amount` a szerver által számolva) és `driver_payments` (amit ténylegesen KIFIZETETT: amount + currency + `bnr_rate` a BNR-ből + `amount_ron` a kifizetéskori árfolyamon + method: cash|bank|card|other + note). A régi `driver_advances` érintetlen (visszamenőleg működik).
2. **`handlers/fleetCompliance.js`** 7 új RPC (Admin/Manager, cégre szűrt, paraméteres SQL, audit): `earningList` / `earningCreate` (kind fehérlista: bonus|diurna|per_diem|salary|premium|holiday|other, cross-tenant védelem a sofőr `company_id` ellenőrzésével a beszúrás ELŐTT, total mindig szerver-oldalon szorzódik → a klienstől kapott érték nem hihető) / `earningDelete`; `paymentList` / `paymentCreate` (a `services/bnr.js` `fetchBnrEurRon`-jából BNR-árfolyam mentése, EUR-nál `amount_ron = amount × bnr_rate`, RON-nál `amount_ron = amount`; method fehérlista → ismeretlen érték `cash`-re) / `paymentDelete`; `getDriverBalance` (járandóság - kifizetés valuta szerint + kombinált RON-egyenleg a MAI BNR-en informatívan). Nincs cross-tenant szivárgás (WHERE `id=$1 AND company_id=$2`).
3. **UI kártyás decont overhaul** (`public/fleet-extra.js` `loadDecont`/`dcLoad` teljesen átépítve, ~500 új sor a KÖZÖS blokkban): fejléc-kártya sofőr-választó + időszak + **élő BNR-árfolyam** sáv; egyenleg-csempék (6 db: járandóság EUR + RON, kifizetve EUR + RON, **hátralék EUR + RON** akcens-csíkkal — piros ha tartozik, zöld ha túlfizetés); két nagy gomb **💵 Részleges** és **✅ Teljes kifizetés**; járandóság-felvitel kártya (kind + label + dátum + qty + unit + currency + note, **live-számoló** `qty × unit = total`, típus szerint intelligens EUR/RON alapérték); két oszlopban a járandóság-lista + kifizetés-lista színes kind-pilulákkal és method-pilulákkal, a kifizetés-lista a mentett BNR-árfolyammal + RON-ekvivalenssel; a legacy menetlevél-blokk (előlegek + készpénz + diurna) a végén megmarad.
4. **Kifizetés-modal**: dedikált teljes-képernyős modal a projekt `.modal-back`/`.modal.glass` mintáján; balra a hátralék EUR + RON kiemelten; jobbra dátum + valuta + összeg + módszer + megjegyzés; **live BNR-előnézet** (EUR beírásra „≈ X RON" a mai BNR-en); teljes kifizetés módra alapból a hátralék automatikusan kitöltve. Egyszeri koppintással rögzít.
5. **Kontrasztos + kártyás CSS** (`public/style.css` végén ~200 új sor, kizárólag `#decontBox` + `#dcPayModal` scope-ban): 6-csempés egyenleg-rács tone-akcens-csíkokkal (ok/warn/danger/info), színes kind-pilula (bonus→borostyán, diurna→kék, per_diem→indigó, salary→zöld, premium→rózsa, holiday→piros, other→szürke), method-pirula, világos + sötét téma egyaránt kiszolgálva, reszponzív (≤640px: 2 oszlopos csempe-rács, ≤420px: 1 oszlop).
6. **i18n** 70+ új kulcs (`fe.dc.bnr*/legacyTitle/balEur/balRon/balCombined`, `fe.de.*` — új-járandóság kártya, kind katalógus + placeholder, `fe.pm.*` — kifizetés modal + lista + method fehérlista), mind RO-alap + HU.
7. **Multi-tenant + input-validáció végig ellenőrzött:** kind + method + currency fehérlista, escape/hossz-korlát (label 120, note 500), NaN védelem (`Number.isFinite`), a kliens-küldte total sosem hasznosul (szerveren szorozzuk), cross-tenant sofőr → 0 sor + „Soferul nu a fost gasit". BNR-hívás best-effort (nem elérhető → `bnr_rate = null`, a kifizetés akkor is mentődik).
8. **Teszt** — `tests/integration/driver-earnings-payments.test.js` (+13 új eset: szerep-kapuk, üres email, cross-tenant védelem, érvénytelen mennyiség, kind fehérlista → other, szerver-oldali total-szorzás, BNR mentés + EUR→RON konverzió, RON alapérték BNR nélkül, method fehérlista, egyenleg + kombinált RON, list company_id-szűrt WHERE, delete cross-tenant védelem). Cache-bust `?v=20260901decont2` (admin.html + manager.html: style.css + i18n.js + fleet-extra.js). **1010 Jest zöld** (997 → 1010, +13), require-sweep 136 modul 0 hiba.
