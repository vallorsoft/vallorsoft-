...
- **`services/scheduler.js`** — új `startServiceDueScheduler` (12 órás): a küszöbön belüli/túllépett szervizekről **push** (`sendPushToRole` Admin/Manager) + **Notifications-központ** (`notify`) + **e-mail** (cég saját feladó-fiók — `getCompanyMailer`, RO). Hetente ismétel (`last_alert_at`). A **`startExpiryScheduler`** is kap **e-mailt** a meglévő push/notify mellé (a lejáró dokumentumokról). Új közös segédek: `_alertEmailWrap` (RO meleg keret) + `_emailAlertToAdmins` (a cég Admin/Manager felhasználóinak, best-effort — feladó-fiók nélkül csendben kihagyja).
