...
**Legújabb kör (2026-06-14 — Ügyfél kérések fül + lebegő fuvarkérés-értesítő):** *(részletes kész-lista: `CHANGELOG.md`)*
...
2. **Új „📋 Ügyfél kérések" fül** (`public/client-requests.js`, `data-tab=client-requests`, `feature-catalog.js`) — a portál-kérések (`inbound_orders` `source='portal'`) **ügyfelenként lenyitható szekcióban, dátum szerint naplózva**; teljes szerkeszthető áru-adatlap + „📄 Kiolvasás" (AI) + „✓ Elfogadás → Disponibil" / „✕ Elvetés". A portál-kérés **többé NEM a „Megrendelések" fülön** (a list-endpoint `?source=`/`?exclude_source=` szűrővel; az approve a teljes áru-adatot átviszi: `suly_kg`/`load_type`/`hossz_cm`/`szel_cm`/`mag_cm`).
