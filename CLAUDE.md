...
**Legújabb kör (2026-06-18 — Tranzakciós e-mail sablon: közvetlen küldés címzettnek + folyamatba kötés):** *(részletes kész-lista: `CHANGELOG.md`)*
1. **`public/templated-email.js`** (ÚJ, közös) — `sendTemplatedEmailDialog({templateKey,keys,vars,toEmail})`: sablon-választós + címzett + `{{változó}}`-mezős dialógus, a meglévő `gas('sendTemplatedEmail')`-re (cég sablonja, KÖZÖS VallorSoft Brevo-feladó, escape-elt változók, `mail_log`).
