...
**Legújabb kör (2026-06-14 — Developer cég-adatexport ZIP hibajavítás):** *(részletes kész-lista: `CHANGELOG.md`)*
1. **Developer export 500-as hiba javítva** (`routes/developer-export.js`) — az `orders` és `order_legs` SQL lekérdezések nem létező oszlopneveket használtak (`rendszam`, `felrako`, `lerako`, `ar`, `ar_valuta`, `cim`, `sorrend` stb.), amelyek PostgreSQL hibát okoztak → minden 📦 export 500-as szerverhiba volt. Javítva a valódi mezőnevekre (`pret`, `loc_incarcare`, `loc_descarcare`, `rendszam_camion`, `leg_number`, `loc_preluare` stb.).
