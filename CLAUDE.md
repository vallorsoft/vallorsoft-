...
Tesztek zöldek (41 Jest). Deploy-teendő: szerver-restart (a `db/*.sql` migrációk, köztük a `db/order-route-geo.sql` és `db/default-trailer-and-weight.sql` automatikusan lefutnak) + böngésző hard refresh.

**Legújabb kör (rakomány-típus kötelező + méretek):**
1. **FTL/LTL kötelező** a fuvar-kiírón ÉS a szerkesztőn (`*` jelölés) — kliens- és szerveroldali (`comCreate`/`comUpdate`) validáció (`validateLoadTypeDims` a handlers/orders.js-ben). Hiányzó típus → hibaüzenet.
