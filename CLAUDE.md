...
**Legújabb kör (2026-06-15 — Landing #pricing: 4 csomagos rács, per-csomag színek, kétnyelvű, ÁFA-megjegyzés, PR #126):** *(részletes kész-lista: `CHANGELOG.md`)*
1. **`public/landing.css`** — `--lp-green` token; `.lp-plan-green/.blue/.indigo/.dark` szín-módosítók `--plan-accent` CSS-változóval; 4 oszlopos rács (1024px → 2×2, 640px → 1 col); `.lp-plan-audience` célközönség-sor; `.lp-pricing-vat` ÁFA-megjegyzés; Pro kártya lift.
2. **`public/landing.js`** — `_cachedPlans` + `renderPricingGrid` újraírás: `p.features` JSONB → bullet-lista; per-csomag szín/badge/CTA; `applyLanguage`-ban re-render; 8 új i18n kulcs (RO+HU): `planAlapAudience`, `planStandardAudience`, `planProAudience`, `planBusinessAudience`, `planStartBtn`, `planContactBtn`, `planPopular`, `pricingVatNote`.
3. **`public/index.html`** — statikus fallback frissítve 4 valós csomagra (Alap/Standard/Pro/Business) helyes tartalommal, CSS osztályokkal és `#lpPricingVat` elemmel.
