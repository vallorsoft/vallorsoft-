...
> **Hiánylista — a 2026-06-13-i ütemterv LEZÁRVA:** **(1) ✅ CI + valódi tesztek** (mock + valós-DB, 101 teszt); **(2) ✅ teherautó-routing váltó** (ORS `driving-hgv`, alap ingyenes OSRM) **+ ✅ valós útdíj váltó** (HERE „Pontos", alap becslés); **(3) ✅ UIT CargoTrack deep-link** (ANAF-integráció helyett, cégenkénti URL-sablon) — *e-Factura a számlázó-providereknél marad (FGO/SmartBill/Oblio); az `efactura_status` megjelenítése a provider-emit oldaláról jövő kis kör*; **(4) ✅ üzemeltetés** (health-check `/healthz`+`/readyz`, strukturált log, opcionális Sentry, opcionális pg_dump backup); **(5) ✅ leg ↔ `orders.email_sofer` szinkron**; **(6) ✅ SaaS-vízvezeték** (csomag-limit kikényszerítés, audit-napló, GDPR export/anonimizálás, Stripe-váz). **Nyitott jövőbeli munka:** ✅ e-Factura státusz automatikus lekérdezés provider-válaszból (elkészült), Stripe éles bekötés (kulcsok + price_xxx — utolsó lépés, nem sürgős), a méretezhető/áthelyezhető táblák kiterjesztése. **RO megfelelőség:** a rendszer megfelel — minden ANAF-kommunikáció (e-Factura SPV-beküldés) a számlázó-providereken (FGO/SmartBill/Oblio stb.) keresztül történik, saját ANAF-kapcsolat NEM kell és NEM is akarunk. Az UIT-kódot sem mi generáljuk — a sofőr/cég a CargoTrack deep-linken keresztül intézi. A GPS→ANAF élő e-Transport-transzmisszió szintén NEM feladatunk (az ANAF API egyébként sem stabil/nyilvános még).
