...
Tesztek zöldek (42 Jest). Deploy-teendő: szerver-restart (a `db/*.sql` migrációk, köztük a `db/order-route-geo.sql` és `db/default-trailer-and-weight.sql` automatikusan lefutnak) + böngésző hard refresh.

## Kétnyelvűség (RO / HU) — KÖTELEZŐ minden új felületen

Az alkalmazás **kétnyelvű: román (RO, alapértelmezett) és magyar (HU)**, kliensoldali váltóval. **Minden új felület/funkció kétnyelvű kell legyen** — soha ne írj be nyers magyar (vagy nyers román) szöveget a UI-ba, mindig az i18n-motoron át.

- **Motor:** `public/i18n.js` — egyetlen `DICT = { 'kulcs': { hu, ro } }` szótár + `t(kulcs, vars)`. A `{n}` típusú változókat a `vars` objektum tölti (`t('kv.selected', { n: 5 })`).
- **HTML statikus szöveg:** `data-i18n="kulcs"` (textContent), `data-i18n-ph` (placeholder), `data-i18n-html` (innerHTML), `data-i18n-title` (title). Az `applyI18n()` automatikusan lefordítja boot-kor és nyelvváltáskor.
- **Kliens JS dinamikus szöveg:** `t('kulcs')` (a `window.t` globális). Minden oldal-JS-ben egy kis `function T(k,v){ return (typeof window.t==='function') ? window.t(k,v) : k; }` segéd, és a renderelt HTML/toast `T('kulcs')`-ot használ.
- **Nyelvváltáskor újrarender:** a dinamikusan renderelt listákat a `window.onLangChange = function(){ ... renderXY(); }` hook frissíti (a statikus DOM-ot az i18n.js intézi). A render-függvényekben a státusz/szöveg **render-időben** forduljon (kulcsot tárolj, ne kész feliratot — ld. portal.js `ST` map `k:` mezővel).
- **Nyelv tárolása:** `localStorage('vs-lang')`, alapértelmezés `'ro'`. A 🇭🇺/🇷🇴 kapcsoló automatikusan a jobb felső sarokba kerül (vagy a `#langSwitch` konténerbe).
- **Betöltés:** minden HTML-be `<script src="/i18n.js?v=…"></script>` a többi kliens-JS ELŐTT.
- **Namespace-ek a DICT-ben:** `common.*`, `nav.*` (sidebar), `form.*` (fuvar-kiíró), `list.*` (fuvarlista), `edit.*` (szerkesztő-modal), `pane.*` (H2 címek), `set.*` (e-mail nyelv), `login.*`, `sofer.*` (sofőr-app), `pt.*` (ügyfél-portál), `cp.*` (alvállalkozói portál), `kv.*` (könyvelői felület), `am.*` (admin/manager dinamikus toastok).
- **E-mailek:** `services/email.js` sablonjai kétnyelvűek (`lang` paraméter, alap `'ro'`); a cég preferált e-mail-nyelve `companies.email_lang` (admin állítja a Beállítások fülön — `getEmailLang`/`setEmailLang`), kiolvasás `lib/companyLang.js` `emailLang(companyId)`. Migráció: `db/email-lang.sql`.
- **Készen (TELJES kétnyelvűség):** minden felület RO/HU. login/register/reset, admin+manager (statikus HTML `ah.*`/`mh.*` + sidebar/űrlapok/lista/szerkesztő/pane-címek + **console-shared.js renderelt HTML** `cs.*`: fuvar-lista sorok, státusz-badge-ek, szerkesztő-modal, raktár/leadás, fizetés, útdíj + admin.js/manager.js `mg.*`), összes kártya-JS (`stats.js` `stat.*`, `planner.js` `pl.*`, `fleet-extra.js` `fl.*`, `billing/invoicing` `bil.*`, `clients` `cl.*`, `email-intake/inbound` `in.*`, `cargotrack` `gps.*`, `uit-panel` `uit.*`), sofőr (statikus + `sofer.*` + `sofer.js`/`sofer-uit.js` `sof.*`), ügyfél-portál (`pt.*`), alvállalkozói portál (`cp.*`), könyvelői felület (`kv.*`), developer (`dev.*` + feature-label fordítás), útvonaltervező (`rp.*`), publikus követő (`track.html` `trk.*`), meghívó/jelszó-emailek.
- **Szerveroldali üzenetek:** `public/i18n-srv.js` (146 nyers magyar szerver-üzenet RO fordítása, `srv.*`); a `tx(huText)` fordított-kereső + a `toast()` automatikus `tx()`-hívása fordítja le őket kliensoldalon (a szerver érintetlen). Betöltve minden szerver-üzenetet mutató oldalon. Néhány futásidőben interpolált üzenet (pl. „Az útvonal nem számolható: …") magyar marad — ezek a kivételek (lásd i18n-srv.js).
- **Motor-API (kártya-JS-ekhez):** `registerI18n(dict)` — önregisztráló namespace-szótár ütközés nélkül (a `__i18nQueue` shimmel a betöltési sorrendtől függetlenül); `I18N.onLang(cb)` — többszörös nyelvváltás-feliratkozó (egy oldalon sok kártya-JS); `tx(s)` — szerver-üzenet fordított keresés.
- **Lefedettség-ellenőrzés:** a referált `data-i18n`/`t('…')` kulcsok mind definiáltak (kulcs-keresztellenőrzés: 0 hiányzó).
