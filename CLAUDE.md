...
> **Hiánylista — a 2026-06-13-i ütemterv LEZÁRVA:** **(1) ✅ CI + valódi tesztek** (mock + valós-DB, 101 teszt); **(2) ✅ teherautó-routing váltó** (ORS `driving-hgv`, alap ingyenes OSRM) **+ ✅ valós útdíj váltó** (HERE „Pontos", alap becslés); **(3) ✅ UIT CargoTrack deep-link** (ANAF-integráció helyett, cégenkénti URL-sablon) — *e-Factura a számlázó-providereknél marad (FGO/SmartBill/Oblio); az `efactura_status` megjelenítése a provider-emit oldaláról jövő kis kör*; **(4) ✅ üzemeltetés** (health-check `/healthz`+`/readyz`, strukturált log, opcionális Sentry, opcionális pg_dump backup); **(5) ✅ leg ↔ `orders.email_sofer` szinkron**; **(6) ✅ SaaS-vízvezeték** (csomag-limit kikényszerítés, audit-napló, GDPR export/anonimizálás, Stripe-váz). **Nyitott jövőbeli munka:** e-Factura státusz-kinyerés provider-válaszból, Stripe éles bekötés (kulcsok + price_xxx), a méretezhető/áthelyezhető táblák kiterjesztése. **RO megfelelőség nyitott (nagy, külön döntés):** SAF-T D406 XML-export (a SAGA/WinMentor CSV megvan), GPS→ANAF élő e-Transport-transzmisszió (2026-os kötelezettség), e-Factura `efactura_status` megjelenítés a provider-válaszból.

**Legújabb kör (RO-megfelelőség + térképes útvonal-előnézet bekapcsolva + ügyfél-portál meghívó-fix):**
1. **RO megfelelőségi kör** (kutatás: ANAF e-Factura, e-Transport/UIT, ANSPDCP/GDPR + fuvarengedélyek — a rendszer már nagyrészt megfelelt; a hézagok pótolva):
   ...
   - *Szándékosan NEM épült (nagy, külön döntés):* SAF-T D406 XML, GPS→ANAF élő e-Transport-transzmisszió. (Az e-Factura SPV-beküldést a számlázó-providerek végzik — saját UBL-motor NEM kell.)
