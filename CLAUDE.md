...
- **Készen (kétnyelvű):** login, admin/manager sidebar + fuvar-kiíró/lista/szerkesztő/pane-címek + toastok, sofőr statikus, ügyfél-portál, alvállalkozói portál, könyvelői felület, meghívó/jelszó-emailek.
- **Hátralévő (folyamatban):** admin/manager renderelt HTML (fuvar-lista sorok, státusz-badge-ek, modalok belseje), `sofer.js` dinamikus szövegek, a kártya-JS-ek (`stats.js`, `planner.js`, `fleet-extra.js`, `*-card.js`), szerveroldali `data.message`-ek.
