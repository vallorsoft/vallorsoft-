...
window.ClientsPage = (function () {
  ...
  function run(root) {
    ...
    (function () {
      ...
      // Az űrlap mezői (a strukturált cím külön: strada/nr_strada/detalii_adresa/oras/judet/cod_postal).
      // A régi 'localitate'/'adresa' nincs külön mező, de mentéskor szinkronizáljuk (lásd clSave).
      const F = ['denumire','tip','cui_cif','reg_com','tara','judet','strada','nr_strada','detalii_adresa','oras','cod_postal','email','telefon','iban','banca','default_tva','valuta','nota','payment_term_days'];
      let editId = null;
      ...
      // Mutató-sáv (vsMetricBand) feltöltése a már lekért ügyfél-listából — csak megjelenítés.
      function renderBand(clients){
        const band = $('clMetricBand');
        if (!band || typeof window.vsMetricBand !== 'function') return;
        const tr = (typeof window.t === 'function') ? window.t : (k => k);
        const pj = clients.filter(c => c.tip === 'PJ').length;
        const pf = clients.filter(c => c.tip === 'PF').length;
        band.innerHTML = window.vsMetricBand([
          { l: tr('cl.bandTotal'), v: clients.length, sub: tr('cl.bandPj') + ': ' + pj },
          { l: tr('cl.bandPj'), v: pj, sub: '' },
          { l: tr('cl.bandPf'), v: pf, sub: '' }
        ]);
      }
      ...
      function openForm(c) {
        ...
        // Visszafelé kompatibilitás: ha a régi rekordnál nincs külön 'oras', essen vissza a 'localitate'-ra.
        if (c && !$('f_oras').value && c.localitate) $('f_oras').value = c.localitate;
      ...
      $('clAnaf').addEventListener('click', async () => {
        ...
        try {
          ...
          // Strukturált cím-mezők kitöltése külön (nem egy összevont sor).
          if (r.street) $('f_strada').value = r.street;
          if (r.streetNumber) $('f_nr_strada').value = r.streetNumber;
          if (r.addressDetails) $('f_detalii_adresa').value = r.addressDetails;
          if (r.locality || r.city) $('f_oras').value = r.locality || r.city;
          if (r.county) $('f_judet').value = r.county;
          if (r.postalCode) $('f_cod_postal').value = r.postalCode;
      ...
      $('clSave').addEventListener('click', async () => {
        ...
        // Régi mezők szinkronizálása a strukturált címből (visszafelé kompatibilis):
        // 'localitate' = oras; 'adresa' = strukturált részek vesszővel összefűzve.
        body.localitate = body.oras || '';
        const utca = body.strada ? (body.nr_strada ? body.strada + ' nr. ' + body.nr_strada : body.strada) : '';
        body.adresa = [utca, body.detalii_adresa, body.oras, body.judet, body.cod_postal].filter(Boolean).join(', ');
        setMsg('Mentés…');
        try {
