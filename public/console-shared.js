...
(window.registerI18n||function(d){(window.__i18nQueue=window.__i18nQueue||[]).push(d);})({
  ...
  'cs.confirmDeleteSwap':{ hu:'Törlöd ezt a váltást?', ro:'Ștergi acest schimb?' },
  'cs.pairedDriverLbl': { hu:'párosított sofőr: ', ro:'șofer asociat: ' },
  'cs.pairedVehicleLbl':{ hu:'párosított jármű: ', ro:'vehicul asociat: ' },
  'cs.pairedTrailerLbl':{ hu:'párosított pótkocsi: ', ro:'semiremorcă asociată: ' },
  'cs.tractorWord':     { hu:'Vontató', ro:'Cap tractor' },
  'cs.trailerWord':     { hu:'Pótkocsi', ro:'Semiremorcă' },
  'cs.addedSuffix':     { hu:' hozzáadva!', ro:' adăugat!' },
  // ── Fuvar CSV-import ──
  'cs.oiClient':        { hu:'Ügyfél', ro:'Client' },
  'cs.oiRef':           { hu:'Referencia', ro:'Referință' },
  'cs.oiLoad':          { hu:'Felrakó', ro:'Încărcare' },
  'cs.oiUnload':        { hu:'Lerakó', ro:'Descărcare' },
  'cs.oiLoadDate':      { hu:'Felrakás dátuma', ro:'Data încărcării' },
  'cs.oiUnloadDate':    { hu:'Lerakás dátuma', ro:'Data descărcării' },
  'cs.oiPrice':         { hu:'Ár', ro:'Preț' },
  'cs.oiKm':            { hu:'Km', ro:'Km' },
  'cs.oiWeight':        { hu:'Súly (kg)', ro:'Greutate (kg)' },
  'cs.oiLoadType':      { hu:'FTL/LTL típus', ro:'Tip FTL/LTL' },
  'cs.oiLength':        { hu:'Hossz (cm)', ro:'Lungime (cm)' },
  'cs.oiWidth':         { hu:'Szélesség (cm)', ro:'Lățime (cm)' },
  'cs.oiHeight':        { hu:'Magasság (cm)', ro:'Înălțime (cm)' },
  'cs.oiTractor':       { hu:'Vontató rendszám', ro:'Nr. înmatriculare cap tractor' },
  'cs.oiTrailer':       { hu:'Pótkocsi rendszám', ro:'Nr. înmatriculare semiremorcă' },
  'cs.oiDriverName':    { hu:'Sofőr neve', ro:'Numele șoferului' },
  'cs.oiDriverEmail':   { hu:'Sofőr e-mail', ro:'E-mail șofer' },
  'cs.oiExtCompany':    { hu:'Külsős cég', ro:'Firmă externă' },
  'cs.oiExtPhone':      { hu:'Külsős telefon', ro:'Telefon extern' },
  'cs.oiCardTitle':     { hu:'📥 Fuvar-CSV import', ro:'📥 Import CSV curse' },
  'cs.oiClose':         { hu:'✕ Bezár', ro:'✕ Închide' },
  'cs.oiIntro':         { hu:'Töltsd fel a gyári/szállítói fuvarlistát (.csv). A rendszer felismeri az elválasztót, és fejléc-név alapján automatikusan párosítja az oszlopokat — ellenőrizd/javítsd, majd importálj. <b>A nem párosított oszlopok nem vesznek el:</b> a fuvar „Importált extra adatok" mezőjébe kerülnek (a szerkesztőben megnézhető).', ro:'Încarcă lista de curse a fabricii/transportatorului (.csv). Sistemul recunoaște separatorul și potrivește automat coloanele după numele antetului — verifică/corectează, apoi importă. <b>Coloanele nepotrivite nu se pierd:</b> ajung în câmpul „Date suplimentare importate" al cursei (vizibil în editor).' },
  'cs.oiFile':          { hu:'CSV fájl', ro:'Fișier CSV' },
  'cs.oiNoneExtra':     { hu:'— (nincs / extra) —', ro:'— (niciun / extra) —' },
  'cs.oiColumn':        { hu:'oszlop ', ro:'coloana ' },
  'cs.oiMapTitle':      { hu:'Oszlop-párosítás — ', ro:'Potrivire coloane — ' },
  'cs.oiRowsSep':       { hu:' sor (elválasztó: ', ro:' rânduri (separator: ' },
  'cs.oiImportBtn':     { hu:' fuvar importálása', ro:' curse de importat' },
  'cs.oiUnmappedCol':   { hu:'⚠️ Nem párosított oszlop (', ro:'⚠️ Coloane nepotrivite (' },
  'cs.oiUnmappedToExtra':{ hu:') → a fuvar „Importált extra adatok" mezőjébe kerül.', ro:') → ajung în câmpul „Date suplimentare importate" al cursei.' },
  'cs.oiAllMapped':     { hu:'✓ Minden oszlop párosítva.', ro:'✓ Toate coloanele potrivite.' },
  'cs.oiExtraCol':      { hu:'+ extra', ro:'+ extra' },
  'cs.oiFieldsWord':    { hu:' mező', ro:' câmpuri' },
  'cs.oiPreview':       { hu:'Előnézet (első 3 sor):', ro:'Previzualizare (primele 3 rânduri):' },
  'cs.oiImporting':     { hu:'Importálás…', ro:'Se importă…' },
  'cs.oiCreated':       { hu:' fuvar létrehozva', ro:' curse create' },
  'cs.oiSkipped':       { hu:' kihagyva', ro:' omise' },
  'cs.oiCreatedOk':     { hu:' fuvar létrehozva.', ro:' curse create.' },
  'cs.oiSkippedRows':   { hu:' sor kihagyva (üres/hibás).', ro:' rânduri omise (goale/eronate).' },
  ...
  // ── FTL/LTL badge title-ök ──
  'cs.dimTitle':        { hu:'Rakomány-méret (h×sz×m cm)', ro:'Dimensiune marfă (L×l×î cm)' },
  'cs.ftlTitle':        { hu:'Full Truck Load — teljes rakomány', ro:'Full Truck Load — marfă completă' },
  'cs.ltlTitle':        { hu:'Less Than Truckload — részrakomány', ro:'Less Than Truckload — marfă parțială' },
  // ── Számla-indikátor title-ök (fuvarlista pipa) ──
  'cs.invStornoed':     { hu:'Számla stornózva: ', ro:'Factură stornată: ' },
  'cs.invInvoiced':     { hu:'Számlázva: ', ro:'Facturat: ' },
  'cs.efacturaLbl':     { hu:' · e-Factura: ', ro:' · e-Factura: ' },
  // ── Egyéb hiba-feliratok (ha a szerver nem ad err-üzenetet) ──
  'cs.downloadErr':     { hu:'Letöltési hiba', ro:'Eroare la descărcare' },
  'cs.uploadErr':       { hu:'Feltöltési hiba', ro:'Eroare la încărcare' },
  'cs.pdfLoadErr':      { hu:'PDF betöltési hiba', ro:'Eroare la încărcarea PDF' },
  'cs.estimateErr':     { hu:'Becslés hiba', ro:'Eroare la estimare' },
  'cs.uitActive':       { hu:'aktív:', ro:'active:' },
  'cs.uitError':        { hu:'hiba:', ro:'erori:' }
});
...
function addStampToPage(){
  if(!pdfDocProxy){ toast(t('am.loadPdfFirst'),'err'); return; }
  if(!savedStampBase64){ toast(t('am.noStamp'),'err'); return; }
  createDraggableItem(savedStampBase64,'stamp');
  toast(t('am.stampAdded'),'ok');
}
...
async function burnAndSaveDoc(){
  ...
  gas('orderDocSaveSigned',[currentDocId,dataUrl]).then(r=>{
    if(r.ok){ toast(t('am.savedToSystem'),'ok'); loadDocList(currentDocOrderId); closeSignModal(); }
    else{ toast(r.err||T('cs.saveErr'),'err'); }
  });
}

function chatGetLabel(roomId,meta,me){
  if(roomId==='manager')return T('cs.chatGroupDriverDisp');
  if(roomId==='admin_manager')return T('cs.chatAdminManager');
...
function createExtDriver(){
  ...
  if(!d.nume&&!d.firma){toast(t('am.driverOrCompanyReq'),'err');return;}
  gas('extDriverCreate',[d]).then(r=>{if(r.ok){toast(t('am.driverAdded'),'ok');['edNume','edFirma','edTelefon','edEmail','edCamion','edRemorca','edNota'].forEach(id=>{document.getElementById(id).value='';});loadExtDrivers();}else{toast(r.err||'Hiba történt','err');}});
}
...
function createInv(){
  ...
  gas('invCreate',[poz, email, nume, tel]).then(function(r){
    if(r&&r.ok){ toast(t('am.code')+r.kod,'ok'); loadInvites();
...
function ensureRouteMapModal(){
  ...
  d.innerHTML='<div class="rmm-card">'
    +'<div class="rmm-head"><b class="text-primary" style="font-size:15px;">'+esc(T('cs.rmTitle'))+'</b>'
    +'<span id="rmmKm" class="badge info" style="margin-left:6px;">—</span>'
    +'<button class="btn ghost" style="margin-left:auto;padding:5px 12px;" onclick="closeRouteMap()">'+esc(T('cs.oiClose'))+'</button></div>'
    +'<div class="rmm-body">'
    +'<div class="rmm-map" id="rmmMap"></div>'
    +'<div class="rmm-side">'
      +'<div class="text-muted" style="font-size:12px;margin-bottom:10px;line-height:1.45;">'+T('cs.rmIntro')+'</div>'
      +'<div id="rmmViaList"></div>'
      +'<div class="rmm-via"><div class="vs-ac-wrap"><input class="input" id="rmmViaInput" placeholder="'+esc(T('cs.rmViaPh'))+'" autocomplete="off"><div class="vs-ac-dd" id="rmmViaInputDD"></div></div>'
      +'<button class="btn ok" style="padding:9px 11px;" onclick="routeMapAddViaFromInput()">➕</button></div>'
      +'<label style="display:flex;align-items:center;gap:7px;font-size:12px;margin-top:10px;cursor:pointer;"><input type="checkbox" id="rmmClickAdd" onchange="_rmClickAdds=this.checked;"> '+esc(T('cs.rmClickAdd'))+'</label>'
      +'<button class="btn ghost" style="width:100%;margin-top:10px;padding:8px;font-size:12px;" onclick="routeMapClearVia()">'+esc(T('cs.rmClearVia'))+'</button>'
    +'</div></div></div>';
  document.body.appendChild(d);
  vsAttachAutocomplete('rmmViaInput','rmmViaInputDD',null);
}
function initRouteLeaflet(){
  if(typeof L==='undefined'){ toast(t('am.mapNotLoaded'),'err'); return; }
...
function routeMapAddVia(which, pt){
  var st=_rmState[which]; if(st.via.length>=7){ toast(t('am.max7Waypoints'),'err'); return; }
...
function loadTypeBadge(t, dims){
  var d = dims ? ' <span class="badge" style="font-size:10px;padding:1px 6px;background:rgba(255,255,255,0.06);color:var(--text-muted);" title="'+esc(T('cs.dimTitle'))+'">📐 '+dims+'</span>' : '';
  if(t==='FTL')return ' <span class="badge info" title="'+esc(T('cs.ftlTitle'))+'" style="font-size:10px;padding:1px 6px;">FTL</span>'+d;
  if(t==='LTL')return ' <span class="badge warn" title="'+esc(T('cs.ltlTitle'))+'" style="font-size:10px;padding:1px 6px;">LTL</span>'+d;
  return d;
}
...
// A label-eket render-időben fordítjuk (lk = i18n-kulcs).
var OI_FIELDS = [
  { k:'client',          lk:'cs.oiClient',      re:/client|ugyfel|ügyf|customer|beneficiar|megrend/i },
  { k:'ref',             lk:'cs.oiRef',         re:/\bref|rendel|order\s*(no|nr|id)|comand|hivatk/i },
  { k:'loc_incarcare',   lk:'cs.oiLoad',        re:/felrak|incarc|loading|^load|pickup|origin|honnan|berak/i },
  { k:'loc_descarcare',  lk:'cs.oiUnload',      re:/lerak|descarc|unload|deliver|dest|hova|kirak/i },
  { k:'data_incarcare',  lk:'cs.oiLoadDate',    re:/(felrak|incarc|load|pickup).*(dat|date|nap)/i },
  { k:'data_descarcare', lk:'cs.oiUnloadDate',  re:/(lerak|descarc|unload|deliv).*(dat|date|nap)/i },
  { k:'pret',            lk:'cs.oiPrice',       re:/\bar\b|ár|pret|price|fuvardij|díj|tarif|fee|amount/i },
  { k:'km',              lk:'cs.oiKm',          re:/\bkm\b|távol|distan|distanta/i },
  { k:'suly_kg',         lk:'cs.oiWeight',      re:/suly|súly|weight|greutate|\bkg\b|tonna|massa/i },
  { k:'load_type',       lk:'cs.oiLoadType',    re:/ftl|ltl|rakomany|rakomány|load.?type|tip.*marf/i },
  { k:'hossz_cm',        lk:'cs.oiLength',      re:/hossz|length|lungim|\blung/i },
  { k:'szel_cm',         lk:'cs.oiWidth',       re:/szel|width|latim|\blat\b/i },
  { k:'mag_cm',          lk:'cs.oiHeight',      re:/magas|height|inalt|\bmag\b/i },
  { k:'rendszam_camion', lk:'cs.oiTractor',     re:/vontat|camion|truck|tractor|kamion/i },
  { k:'rendszam_remorca',lk:'cs.oiTrailer',     re:/potkocs|pótkocs|remorc|trailer|utánf/i },
  { k:'nume_sofer',      lk:'cs.oiDriverName',  re:/sofer|sofőr|driver|conducator/i },
  { k:'email_sofer',     lk:'cs.oiDriverEmail', re:/e-?mail/i },
  { k:'firma_extern',    lk:'cs.oiExtCompany',  re:/extern|subcontr|transportator|alváll/i },
  { k:'telefon_extern',  lk:'cs.oiExtPhone',    re:/tel|phone|telefon|mobil/i },
];
// label getter: a régi .label helyett mindig render-időben fordít
function oiLabel(fl){ return fl.lk ? T(fl.lk) : (fl.label||fl.k); }
...
function ensureOrderImportModal(){
  ...
  d.innerHTML='<div class="oi-card">'
    +'<div class="oi-head"><b class="text-primary" style="font-size:15px;">'+esc(T('cs.oiCardTitle'))+'</b>'
    +'<button class="btn ghost" style="margin-left:auto;padding:5px 12px;" onclick="closeOrderImport()">'+esc(T('cs.oiClose'))+'</button></div>'
    +'<div class="oi-body">'
    +'<div class="text-muted" style="font-size:12px;margin-bottom:10px;line-height:1.5;">'+T('cs.oiIntro')+'</div>'
    +'<div class="field" style="max-width:420px;"><label>'+esc(T('cs.oiFile'))+'</label><input class="input" type="file" id="oiFile" accept=".csv,.txt" onchange="orderImportParse()"></div>'
...
function orderImportParse(){
  ...
  reader.onload=function(e){
    ...
    if(lines.length<2){ toast(t('am.csvEmpty'),'err'); return; }
    ...
    var opts='<option value="">'+esc(T('cs.oiNoneExtra'))+'</option>'+_oiHeader.map(function(h,i){ return '<option value="'+i+'">'+esc(h||(T('cs.oiColumn')+(i+1)))+'</option>'; }).join('');
    var cells=OI_FIELDS.map(function(fl){
      return '<div class="field" style="margin:0;"><label>'+esc(oiLabel(fl))+'</label>'
        +'<select class="select" id="oiCol_'+fl.k+'" onchange="orderImportPreview()">'+opts+'</select></div>';
    }).join('');
    document.getElementById('oiMapping').innerHTML=
      '<div class="glass-soft" style="padding:14px;">'
      +'<div class="text-primary" style="font-size:13px;font-weight:700;margin-bottom:10px;">'+esc(T('cs.oiMapTitle'))+_oiRows.length+esc(T('cs.oiRowsSep'))+'<code>'+(delim==='\t'?'TAB':esc(delim))+'</code>)</div>'
      +'<div class="oi-map-grid">'+cells+'</div>'
      +'<div style="display:flex;gap:10px;align-items:center;margin-top:14px;flex-wrap:wrap;">'
      +'<button class="btn primary" onclick="orderImportRun(this)">📥 '+_oiRows.length+esc(T('cs.oiImportBtn'))+'</button>'
...
function orderImportPreview(){
  ...
  if(un) un.innerHTML = unmapped.length
    ? T('cs.oiUnmappedCol')+unmapped.length+'): <b>'+esc(unmapped.join(', ').slice(0,120))+'</b>'+T('cs.oiUnmappedToExtra')
    : T('cs.oiAllMapped');
  // előnézet: első 3 sor a párosítás szerint
  var cols=OI_FIELDS.filter(function(fl){ return oiColIndex(fl.k)>=0; });
  var head='<tr>'+cols.map(function(fl){ return '<th>'+esc(oiLabel(fl))+'</th>'; }).join('')+(unmapped.length?'<th>'+esc(T('cs.oiExtraCol'))+'</th>':'')+'</tr>';
  var body=_oiRows.slice(0,3).map(function(r){
    ...
    return '<tr>'+cols.map(function(fl){ var v=o[fl.k]; return '<td>'+esc(v==null||v===''?'—':String(v))+'</td>'; }).join('')
      +(unmapped.length?'<td class="text-muted">'+esc(o.import_extra?Object.keys(o.import_extra).length+T('cs.oiFieldsWord'):'—')+'</td>':'')+'</tr>';
  }).join('');
  if(pv) pv.innerHTML='<div class="text-muted" style="font-size:11px;margin-bottom:4px;">'+esc(T('cs.oiPreview'))+'</div><div class="oi-prev"><table><thead>'+head+'</thead><tbody>'+body+'</tbody></table></div>';
}
function orderImportRun(btn){
  if(!_oiRows.length){ toast(t('am.uploadCsvFirst'),'err'); return; }
  var rows=_oiRows.map(oiBuildRow);
  if(btn){ btn.disabled=true; btn.textContent=T('cs.oiImporting'); }
  gas('bulkCreateOrders',[{rows:rows}]).then(function(r){
    if(btn){ btn.disabled=false; btn.textContent='📥 '+_oiRows.length+T('cs.oiImportBtn'); }
    var res=document.getElementById('oiResult');
    if(r&&r.ok){
      toast(t('am.importDone')+r.inserted+T('cs.oiCreated')+(r.skipped?(' · '+r.skipped+T('cs.oiSkipped')):''),'ok');
      if(res) res.innerHTML='<div class="glass-soft" style="padding:12px;border:1px solid rgba(34,197,94,0.4);">'
        +'<b style="color:var(--status-ok);">✅ '+r.inserted+T('cs.oiCreatedOk')+'</b>'
        +(r.skipped?(' <span class="text-muted">'+r.skipped+T('cs.oiSkippedRows')+'</span>'):'')
        +'</div>';
      if(typeof loadOrders==='function') loadOrders();
    } else { toast((r&&r.err)||'Import hiba','err'); }
  }).catch(function(){ if(btn){btn.disabled=false;} toast(t('am.importErr'),'err'); });
}
...
function createOrder(){
  ...
  if(!p.client){toast(t('am.clientNameReq'),'err');return;}
  if(!p.load_type){toast(t('am.pickLoadType'),'err');return;}
  if(p.load_type==='LTL' && (!p.hossz_cm||!p.szel_cm||!p.mag_cm)){toast(t('am.ltlDimsReq'),'err');return;}
  gas('comCreate',[p]).then(r=>{
    if(r&&r.ok){
      let extra='';
      if(r.paired_driver)extra=' · 👤 '+T('cs.pairedDriverLbl')+r.paired_driver;
      else if(r.paired_vehicle)extra=' · 🚛 '+T('cs.pairedVehicleLbl')+r.paired_vehicle;
      if(r.paired_trailer)extra+=' · 🚚 '+T('cs.pairedTrailerLbl')+r.paired_trailer;
      toast(t('am.orderSaved')+r.id+extra,'ok');
      loadOrders();
...
function createVehicle(tip){
  ...
  if(!v.rendszam){toast(t('am.plateReq'),'err');return;}
  gas('vehicleCreate',[v]).then(r=>{if(r.ok){toast((tip==='Vontato'?T('cs.tractorWord'):T('cs.trailerWord'))+T('cs.addedSuffix'),'ok');['Rendszam','Marca','Model','An','Nota'].forEach(f=>{document.getElementById(prefix+f).value='';});if(tip==='Potkocsi'&&typeof resetTrailerFormDefaults==='function')resetTrailerFormDefaults();loadVehicles();}else{toast(r.err||T('cs.errorOccurred'),'err');}});
}

function deleteExtDriver(id){
  const d=extDriverCache.find(x=>x.id===id);
  const nev=d?(d.nume||d.firma||'?'):'?';
  if(!confirm(T('cs.confirmDelete',{name:nev})))return;
  gas('extDriverDelete',[id]).then(r=>{if(r.ok){toast(t('am.deletedX'),'ok');loadExtDrivers();}else{toast(r.err||'Hiba történt','err');}});
}

function deleteLeg(legId) {
  if (!confirm(T('cs.confirmDeleteSwap'))) return;
  gas('deleteOrderLeg', [legId]).then(function(r) {
    if (r && r.ok) {
      toast(t('am.deleted'),'ok');
...
function deleteVehicle(id){
  ...
  if(!confirm(T('cs.confirmDeleteVehicle',{plate:rendszam})))return;
  gas('vehicleDelete',[id]).then(r=>{if(r.ok){toast(t('am.deletedX'),'ok');loadVehicles();}else{toast(r.err||'Hiba történt','err');}});
}
...
function editExtDriver(id){
  const d=extDriverCache.find(x=>x.id===id);
  if(!d){toast(t('am.driverNotFound'),'err');return;}
...
function editVehicle(id){
  const v=vehicleCache.find(x=>x.id===id);
  if(!v){toast(t('am.vehicleNotFound'),'err');return;}
  ...
  fetch('/api/firebase-config')
    ...
    .then(function(cfg){
      if(!cfg||!cfg.apiKey){
        ...
        if(el)el.textContent=T('cs.chatConfigMissing');
        return;
      }
    ...
    .catch(function(){
      var el=document.getElementById('chatLoadMsg');
      if(el)el.textContent=T('cs.chatUnavailable');
    });
}
...
function loadBorderLogs(){gas('getBorderLogs').then(list=>{document.querySelector('#tblBorderLogs tbody').innerHTML=list.map(l=>`<tr><td>${l.created_at?new Date(l.created_at).toLocaleString('hu-HU'):'—'}</td><td>${esc(l.nume_sofer||l.email_sofer||'—')}</td><td><span class="badge warn">${esc(l.tip||'—')}</span></td><td>${l.gps_lat?('📍 '+parseFloat(l.gps_lat).toFixed(4)+', '+parseFloat(l.gps_lng).toFixed(4)):esc(T('cs.gpsNa'))}</td></tr>`).join('');});}
...
function loadDriverUploadedDocs() {
  ...
  if (container) container.innerHTML = '<div style="text-align:center;color:var(--muted);padding:40px;"><div class="spinner" style="margin:0 auto 10px;"></div>'+esc(T('cs.loadingDots'))+'</div>';
  gas('getDriverDocs').then(function(list) {
    ...
    if (sel) {
      ...
      sel.innerHTML = '<option value="">'+esc(T('cs.allDrivers'))+'</option>' + sofors.map(function(s){return '<option value="'+esc(s)+'">'+esc(s)+'</option>';}).join('');
    }
    renderDocGroups();
  });
}
...
function loadExtDrivers(){
  gas('extDriverList').then(list=>{
    ...
    if(list.length===0){tb.innerHTML='<tr><td colspan="8" style="text-align:center;color:var(--muted);">'+esc(T('cs.noExtDriver'))+'</td></tr>';return;}
    tb.innerHTML=list.map(d=>`<tr><td>${esc(d.nume||'—')}</td><td>${esc(d.firma||'—')}</td><td>${esc(d.telefon||'—')}</td><td>${esc(d.email||'—')}</td><td>${esc(d.rendszam_camion||'—')}</td><td>${esc(d.rendszam_remorca||'—')}</td><td>${esc(d.nota||'—')}</td><td><button class="btn primary" style="padding:4px 10px;font-size:12px;" onclick="editExtDriver(${d.id})">${esc(T('cs.editShort'))}</button> <button class="btn danger" style="padding:4px 10px;font-size:12px;" onclick="deleteExtDriver(${d.id})">${esc(T('cs.delete'))}</button></td></tr>`).join('');
  }).catch(function(e){ console.error('loadExtDrivers hiba:', e); toast(t('am.loadErr'),'err'); });
}
...
function loadInternalDrivers(){
  ...
  gas('getDriverVehicleAssignments').then(function(r){
    ...
    if(!tbody) return;
    if(!r || !r.ok){ tbody.innerHTML='<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:24px;">'+esc(T('cs.intDrvLoadErr'))+'</td></tr>'; return; }
    var list = r.drivers||[];
    var vehicles = r.vehicles||[];
    var trailers = r.trailers||[];
    if(!list.length){
      tbody.innerHTML='<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:24px;">'+esc(T('cs.noIntDriver'))+'</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = list.map(function(d, i){
      ...
      var opts = '<option value="">'+esc(T('cs.noVehicleDash'))+'</option>' + vehicles.map(function(v){
      ...
      var gpsBadge = myVeh
        ? (myVeh.has_gps
            ? ' <span class="badge ok" title="'+esc(T('cs.gpsLinked'))+'">'+esc(T('cs.gpsBadge'))+'</span>'
            : ' <span class="badge warn" title="'+esc(T('cs.noGpsLink'))+'">'+esc(T('cs.noGps'))+'</span>')
        : '';
      // Alapértelmezett pótkocsi a hozzárendelt vontatóhoz (auto-párosítás
      // fuvar-kiíráskor) — csak ha a sofőrnek van vontatója.
      var trailerSel = '';
      if(myVeh){
        var topts = '<option value="">'+esc(T('cs.noDefTrailer'))+'</option>' + trailers.map(function(t){
        ...
        trailerSel = '<div style="margin-top:5px;">'
          ...
          +'title="'+esc(T('cs.defTrailerTitle'))+'" '
      ...
      return '<tr>'
        ...
        +'</td>'
        +'<td>'
        +'<button class="btn ghost" style="padding:4px 10px;font-size:12px;" onclick="editUser(window._vsIntDrvCache['+i+'])">'+esc(T('cs.edit'))+'</button> '
        +'<button class="btn danger" style="padding:4px 10px;font-size:12px;" onclick="deleteUser(window._vsIntDrvCache['+i+'].email,window._vsIntDrvCache['+i+'].nume)">'+esc(T('cs.delete'))+'</button>'
        +'</td>'
        +'</tr>';
    }).join('');
  }).catch(function(e){ console.error('loadInternalDrivers hiba:', e); toast(t('am.loadErr'),'err'); });
}
...
function loadClientPortalAccess(){
  ...
  if(!(window._vsFeatures && window._vsFeatures['client-portal']===true)){
    box.innerHTML='<div class="glass" style="padding:14px 18px;"><div class="text-muted" style="font-size:12.5px;">'+T('cs.cpDisabled')+'</div></div>';
    return;
  }
  box.innerHTML='<div class="glass" style="padding:18px 20px;"><div style="font-size:16px;font-weight:800;margin-bottom:4px;">'+esc(T('cs.cpTitle'))+'</div>'
    +'<div class="text-muted" style="font-size:12.5px;margin-bottom:14px;">'+T('cs.cpSub')+'</div>'
    +'<div style="display:flex;gap:10px;align-items:end;flex-wrap:wrap;margin-bottom:16px;">'
    +'<div class="field" style="margin:0;min-width:200px;"><label>'+esc(T('cs.cpClient'))+'</label><select class="select" id="cpClientSel"><option value="">'+esc(T('cs.cpChooseClient'))+'</option></select></div>'
    +'<div class="field" style="margin:0;min-width:200px;"><label>'+esc(T('cs.cpContactEmail'))+'</label><input class="input" id="cpEmail" placeholder="pl. logisztika@gyar.ro"></div>'
    +'<div class="field" style="margin:0;min-width:150px;"><label>'+esc(T('cs.cpName'))+'</label><input class="input" id="cpNev"></div>'
    +'<button class="btn ok" style="height:42px;" onclick="cpInvite()">'+esc(T('cs.cpSendInvite'))+'</button>'
    +'</div>'
    +'<div id="cpInviteLink"></div>'
    +'<div id="cpList"><div class="text-muted" style="font-size:12px;">'+esc(T('cs.loading'))+'</div></div>'
    +'</div>';
  ...
  fetch('/api/clients',{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(list){
    ...
    sel.innerHTML='<option value="">'+esc(T('cs.cpChooseClient'))+'</option>'+_cpClients.map(function(c){
...
function cpRefresh(){
  gas('clientPortalList').then(function(r){
    ...
    if(!r||!r.ok){ el.innerHTML='<div class="text-muted" style="font-size:12px;">'+esc(T('cs.loadErr2'))+'</div>'; return; }
    var items=r.items||[];
    if(!items.length){ el.innerHTML='<div class="text-muted" style="font-size:12px;padding:6px 0;">'+esc(T('cs.cpNoAccess'))+'</div>'; return; }
    window._cpItems=items;
    el.innerHTML=items.map(function(it,i){
      var status = it.pending_invite ? '<span class="badge warn">'+esc(T('cs.cpInviteSentBadge'))+'</span>'
        : (it.activ ? '<span class="badge ok">'+esc(T('cs.cpActive'))+'</span>' : '<span class="badge err">'+esc(T('cs.cpBlocked'))+'</span>');
      var last = it.last_login ? (T('cs.cpLastLogin')+String(it.last_login).replace('T',' ').slice(0,16)) : T('cs.cpNeverLoggedIn');
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border,rgba(255,255,255,.08));border-radius:10px;margin-bottom:6px;background:rgba(255,255,255,.02);">'
        ...
        +status
        +'<button class="btn ghost" style="padding:6px 10px;font-size:12px;" onclick="cpToggle('+i+')">'+(it.activ?esc(T('cs.cpBlock')):esc(T('cs.cpActivate')))+'</button>'
        +'</div>';
    }).join('');
  });
}
function cpInvite(){
  ...
  if(!cid){ toast(t('am.pickClient'),'err'); return; }
  if(!email){ toast(t('am.giveContactEmail'),'err'); return; }
  gas('clientPortalInvite',[{client_id:cid, email:email, nev:nev}]).then(function(r){
    if(r&&r.ok){
      toast(r.emailed?T('cs.cpInviteSent'):T('cs.cpInviteCreated'),'ok');
      ...
      if(lb) lb.innerHTML='<div class="glass-soft" style="padding:10px 12px;margin-bottom:12px;border:1px solid rgba(34,197,94,.4);font-size:12px;">'
        +(r.emailed?T('cs.cpInviteSentMail'):'')+T('cs.cpPwLinkCopy')+'<br><code style="word-break:break-all;color:var(--text-primary);">'+esc(r.link)+'</code></div>';
...
function cpToggle(i){
  ...
  gas('clientPortalSetActive',[it.id, !it.activ]).then(function(r){
    if(r&&r.ok){ toast(it.activ?T('cs.cpBlockedWord'):T('cs.cpActivatedWord'),'ok'); cpRefresh(); }
    else toast((r&&r.err)||'Hiba','err');
  });
}
...
function renderTollBreak(tg){
  ...
  var rows=tg.byCountry.map(function(c){
    return '<tr><td>'+(CC_FLAG[c.cc]||'')+' '+esc(c.name||c.cc)+'</td><td style="text-align:right;">'+c.km+' km</td>'
      +'<td style="text-align:right;color:var(--muted);">'+(c.mode==='vignette'?T('cs.tollVignette'):((c.eur_per_km!=null?c.eur_per_km:'')+' €/km'))+'</td>'
  ...
  box.innerHTML='<div class="glass-soft" style="padding:10px 12px;border:1px solid rgba(245,158,11,.35);">'
    +'<div class="text-muted" style="font-size:11.5px;font-weight:700;margin-bottom:6px;">'+esc(T('cs.tollByCountry'))+(tg.pending?T('cs.tollPartial'):'')+'</div>'
    +'<table style="width:100%;border-collapse:collapse;font-size:12px;">'
    +'<tbody>'+rows+'<tr><td style="font-weight:800;border-top:1px solid var(--border,rgba(255,255,255,.1));padding-top:6px;">'+esc(T('cs.total'))+'</td><td colspan="2" style="border-top:1px solid var(--border,rgba(255,255,255,.1));"></td>'
...
function estimateOrderToll(){
  if(!_oeOrderId){ toast(t('am.saveOrderFirst'),'err'); return; }
  toast(t('am.tollEstimating'),'ok');
  gas('estimateToll',[_oeOrderId]).then(function(r){
    if(r&&r.ok){
      ...
      toast(t('am.estimatedToll')+(r.toll?r.toll.total:0)+' €'+T('cs.modifiable'),'ok');
    } else toast((r&&r.err)||T('cs.estimateErr'),'err');
  });
}
...
function openTollRates(){
  ...
  gas('getTollRates').then(function(r){
    ...
    if(!r||!r.ok){ body.innerHTML='<div class="text-muted" style="padding:14px;">'+esc(T('cs.loadErr2'))+'</div>'; return; }
    window._trRates=r.rates||[];
    body.innerHTML='<table style="width:100%;border-collapse:collapse;font-size:13px;"><thead><tr>'
      +'<th style="text-align:left;padding:6px 8px;color:var(--muted);">'+esc(T('cs.colCountry'))+'</th><th style="text-align:left;padding:6px 8px;color:var(--muted);">'+esc(T('cs.colType'))+'</th>'
      +'<th style="text-align:left;padding:6px 8px;color:var(--muted);">€/km</th><th style="text-align:left;padding:6px 8px;color:var(--muted);">'+esc(T('cs.colVignetteEur'))+'</th></tr></thead><tbody>'
      +window._trRates.map(function(rt,i){
        return '<tr style="border-top:1px solid var(--border,rgba(255,255,255,.08));">'
          +'<td style="padding:6px 8px;">'+(CC_FLAG[rt.cc]||'')+' '+esc(rt.name)+(rt.custom?' <span class="badge ok" style="font-size:9px;">'+esc(T('cs.custom'))+'</span>':'')+'</td>'
          +'<td style="padding:6px 8px;"><select class="select" id="tr_mode_'+i+'" style="padding:5px 6px;font-size:12px;"><option value="perkm"'+(rt.mode==='perkm'?' selected':'')+'>'+esc(T('cs.modePerKm'))+'</option><option value="vignette"'+(rt.mode==='vignette'?' selected':'')+'>'+esc(T('cs.tollVignette'))+'</option></select></td>'
...
function ensureTollRatesModal(){
  ...
  d.innerHTML='<div class="glass" style="width:min(680px,100%);background:var(--bg-panel-raised,#141c25);">'
    ...
    +'<b class="text-primary" style="font-size:15px;">'+T('cs.tollRatesTitle')+'</b>'
    +'<button class="btn ghost" style="margin-left:auto;padding:5px 12px;" onclick="closeTollRates()">✕</button></div>'
    +'<div style="padding:14px 18px;"><div class="text-muted" style="font-size:12px;margin-bottom:10px;">'+esc(T('cs.tollRatesHint'))+'</div>'
    +'<div id="trBody"></div>'
    +'<div style="margin-top:14px;display:flex;gap:8px;"><button class="btn primary" onclick="saveTollRatesUi()">'+esc(T('cs.saveRates'))+'</button>'
    +'<button class="btn ghost" onclick="closeTollRates()">'+esc(T('cs.cancel'))+'</button></div></div></div>';
...
function saveTollRatesUi(){
  ...
  gas('saveTollRates',[rates]).then(function(r){
    if(r&&r.ok){ toast(t('am.ratesSaved'),'ok'); closeTollRates(); }
    else toast((r&&r.err)||'Hiba','err');
  });
}
...
function oeUpdateMargin(){
  ...
  if(cost>0){ var m=pret-cost; el.innerHTML=esc(T('cs.margin'))+'<b style="color:'+(m>=0?'var(--status-ok)':'var(--status-danger)')+';">'+(m>=0?'+':'')+Math.round(m)+' €</b>'; }
  else el.textContent='';
}
  ...
  gas('carrierList').then(function(r){
    ...
    var rows=items.map(function(c,i){
      ...
      if(c.cmr_insurance_expiry){ var d=new Date(c.cmr_insurance_expiry); var days=Math.round((d-new Date())/86400000);
        cmr = days<0 ? '<span class="badge err">'+esc(T('cs.cmrExpired'))+'</span>' : days<30 ? '<span class="badge warn">'+days+esc(T('cs.daysWord'))+'</span>' : '<span class="badge ok">'+String(c.cmr_insurance_expiry).slice(0,7)+'</span>'; }
      var ob=Math.round(parseFloat(c.open_balance)||0);
      return '<tr style="'+(!c.aktiv?'opacity:.5;':'')+'">'
        +'<td><b class="text-primary">'+esc(c.nev)+'</b>'+(c.portal_users?' <span class="badge ok" style="font-size:9px;">'+esc(T('cs.portalBadge'))+'</span>':'')+'</td>'
        +'<td>'+esc(c.cui||'—')+'</td><td>'+(c.payment_term_days||30)+esc(T('cs.daysWord'))+'</td><td>'+cmr+'</td>'
        +'<td style="text-align:right;color:'+(ob>0?'#ff6b75':'inherit')+';font-weight:700;">'+ob+' €</td>'
        +'<td style="white-space:nowrap;">'
        +'<button class="btn ghost" style="padding:4px 9px;font-size:12px;" onclick="carrierEditUi('+i+')">'+esc(T('cs.editShort'))+'</button> '
        +'<button class="btn ghost" style="padding:4px 9px;font-size:12px;" onclick="carrierInvitePrompt('+c.id+')" title="'+esc(T('cs.carrierPortalInvite'))+'">🔑</button> '
        +'<button class="btn danger" style="padding:4px 9px;font-size:12px;" onclick="carrierDeleteUi('+c.id+')">✕</button></td></tr>';
    }).join('');
    box.innerHTML='<div class="glass" style="padding:20px;"><div style="font-size:16px;font-weight:800;margin-bottom:4px;">'+esc(T('cs.carriers'))+'</div>'
      +'<div class="text-muted" style="font-size:12.5px;margin-bottom:14px;">'+esc(T('cs.carriersSub'))+'</div>'
      +'<div class="grid-3" style="margin-bottom:12px;">'
      +'<div class="field"><label>'+esc(T('cs.companyName'))+'</label><input class="input" id="caNev"></div>'
      +'<div class="field"><label>'+esc(T('cs.cuiTax'))+'</label><input class="input" id="caCui"></div>'
      +'<div class="field"><label>'+esc(T('cs.email'))+'</label><input class="input" id="caEmail"></div>'
      +'<div class="field"><label>'+esc(T('cs.phone'))+'</label><input class="input" id="caTel"></div>'
      +'<div class="field"><label>'+esc(T('cs.paymentTermDays'))+'</label><input class="input" id="caTerm" type="number" value="30"></div>'
      +'<div class="field"><label>'+esc(T('cs.cmrExpiry'))+'</label><input class="input" id="caCmr" type="date"></div>'
      +'<div class="field"><label>'+esc(T('cs.iban'))+'</label><input class="input" id="caIban"></div>'
      +'<div class="field" style="grid-column:span 2;"><label>'+esc(T('cs.note'))+'</label><input class="input" id="caNota"></div>'
      +'</div>'
      +'<input type="hidden" id="caId"><button class="btn primary" onclick="carrierSaveUi()">'+esc(T('cs.saveCarrier'))+'</button> <button class="btn ghost" onclick="carrierFormReset()">'+esc(T('cs.newEmpty'))+'</button>'
      +'<div id="carrierInviteLink" style="margin-top:12px;"></div>'
      +'<table class="table" style="margin-top:16px;"><thead><tr><th>'+esc(T('cs.colCompany'))+'</th><th>CUI</th><th>'+esc(T('cs.colPayTerm'))+'</th><th>'+esc(T('cs.colCmr'))+'</th><th style="text-align:right;">'+esc(T('cs.colOpenBalance'))+'</th><th>'+esc(T('cs.whColAction'))+'</th></tr></thead>'
      +'<tbody>'+(rows||'<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:14px;">'+esc(T('cs.noCarrier'))+'</td></tr>')+'</tbody></table></div>';
  });
}
...
function carrierSaveUi(){
  ...
  if(!p.nev){ toast(t('am.companyNameReq'),'err'); return; }
  gas('carrierSave',[p]).then(function(r){ if(r&&r.ok){ toast(t('am.carrierSaved'),'ok'); carrierFormReset(); loadCarriers(); loadCarrierAp(); } else toast((r&&r.err)||'Hiba','err'); });
}
function carrierDeleteUi(id){
  if(!confirm(T('cs.confirmDeleteCarrier'))) return;
  gas('carrierDelete',[id]).then(function(r){ if(r&&r.ok){ toast(t('am.deleted'),'ok'); loadCarriers(); } else toast((r&&r.err)||T('cs.cannotDelete'),'err'); });
}
function carrierInvitePrompt(carrierId){
  var email=prompt(T('cs.carrierEmailPrompt')); if(!email) return;
  gas('carrierPortalInvite',[{carrier_id:carrierId, email:email.trim()}]).then(function(r){
    if(r&&r.ok){ toast(r.emailed?T('cs.portalInviteSent'):T('cs.inviteReadyCopy'),'ok');
      var lb=document.getElementById('carrierInviteLink');
      if(lb) lb.innerHTML='<div class="glass-soft" style="padding:10px 12px;border:1px solid rgba(34,197,94,.4);font-size:12px;">'+(r.emailed?T('cs.sentByEmail'):'')+T('cs.pwSetLink')+'<br><code style="word-break:break-all;color:var(--text-primary);">'+esc(r.link)+'</code></div>';
      loadCarriers();
    } else toast((r&&r.err)||'Hiba','err');
  });
}
  ...
  gas('carrierInvoiceList').then(function(r){
    ...
    var rows=items.map(function(it){
      ...
      if(it.due_date){ var days=Math.round((new Date(it.due_date)-new Date())/86400000);
        due = days<0?('<span class="badge err">'+esc(T('cs.apOverdueBy'))+(-days)+esc(T('cs.daysWord'))+'</span>'):days<=7?('<span class="badge warn">'+days+esc(T('cs.daysWord'))+'</span>'):String(it.due_date).slice(0,10); }
      var stB = it.status==='paid'?'<span class="badge ok">'+esc(T('cs.apPaid'))+'</span>':it.status==='partial'?'<span class="badge warn">'+esc(T('cs.apPartial'))+' ('+Math.round(it.paid_amount)+')</span>':'<span class="badge err">'+esc(T('cs.apToPay'))+'</span>';
      ...
      return '<tr><td>'+esc(it.carrier_nev||'')+'</td><td>'+esc(it.invoice_number||'—')+'</td>'
        ...
        +'<td style="white-space:nowrap;">'+(it.status!=='paid'?'<button class="btn ok" style="padding:4px 9px;font-size:12px;" onclick="carrierInvoicePayUi('+it.id+','+rem+')">'+esc(T('cs.apPaid'))+'</button> ':'')
        +'<button class="btn danger" style="padding:4px 9px;font-size:12px;" onclick="carrierInvoiceDeleteUi('+it.id+')">✕</button></td></tr>';
    }).join('');
    box.innerHTML='<div class="glass" style="padding:20px;"><div style="font-size:16px;font-weight:800;margin-bottom:10px;">'+esc(T('cs.apTitle'))+'</div>'
      +'<div class="dash-stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:14px;">'
      +apTile(T('cs.apOpenTotal'), Math.round(s.open_total||0)+' €', '#ff6b75')+apTile(T('cs.apDueSoon'), Math.round(s.due_soon||0)+' €', '#fbbf24')
      +apTile(T('cs.apOverdue'), Math.round(s.overdue||0)+' €', (s.overdue>0?'#ff6b75':''))+apTile(T('cs.apOpenInvoice'), (s.open_cnt||0)+T('cs.apPcs'),'')+'</div>'
      +'<div style="display:flex;gap:8px;align-items:end;flex-wrap:wrap;margin-bottom:10px;">'
      +'<div class="field" style="margin:0;min-width:170px;"><label>'+esc(T('cs.carrier'))+'</label><select class="select" id="ciCarrier" onchange="apLoadOrders()"><option value="">—</option></select></div>'
      +'<div class="field" style="margin:0;"><label>'+esc(T('cs.invoiceNo'))+'</label><input class="input" id="ciNum" style="max-width:140px;"></div>'
      +'<div class="field" style="margin:0;"><label>'+esc(T('cs.issueDate'))+'</label><input class="input" id="ciIssue" type="date" style="max-width:150px;"></div>'
      +'<div class="field" style="margin:0;"><label>'+esc(T('cs.dueDateShort'))+'</label><input class="input" id="ciDue" type="date" style="max-width:150px;"></div>'
      +'<div class="field" style="margin:0;"><label>'+esc(T('cs.amount'))+'</label><input class="input" id="ciAmount" type="number" style="max-width:110px;"></div>'
      +'<div class="field" style="margin:0;"><label>'+esc(T('cs.currency'))+'</label><select class="select" id="ciCurr" style="max-width:90px;"><option>EUR</option><option>RON</option><option>HUF</option><option>PLN</option></select></div>'
      +'<button class="btn primary" style="height:42px;" onclick="carrierInvoiceSaveUi()">'+esc(T('cs.recordInvoice'))+'</button>'
      +'</div>'
      +'<div class="field" id="ciOrdersWrap" style="display:none;"><label>'+esc(T('cs.whichOrders'))+'</label><select class="select" id="ciOrders" multiple size="3" style="height:auto;"></select></div>'
      +'<table class="table" style="margin-top:12px;"><thead><tr><th>'+esc(T('cs.carrier'))+'</th><th>'+esc(T('cs.colInvoice'))+'</th><th>'+esc(T('cs.colOrders'))+'</th><th style="text-align:right;">'+esc(T('cs.amount'))+'</th><th>'+esc(T('cs.colDue'))+'</th><th>'+esc(T('cs.colState'))+'</th><th>'+esc(T('cs.whColAction'))+'</th></tr></thead>'
      +'<tbody>'+(rows||'<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:14px;">'+esc(T('cs.noApInvoice'))+'</td></tr>')+'</tbody></table></div>';
...
function carrierInvoiceSaveUi(){
  ...
  if(!p.carrier_id){ toast(t('am.pickCarrier'),'err'); return; }
  gas('carrierInvoiceSave',[p]).then(function(r){ if(r&&r.ok){ toast(T('cs.invoiceRecorded'),'ok'); ['ciNum','ciAmount'].forEach(function(i){document.getElementById(i).value='';}); loadCarrierAp(); loadCarriers(); } else toast((r&&r.err)||'Hiba','err'); });
}
function carrierInvoicePayUi(id, rem){
  var v=prompt(T('cs.paidFullPrompt',{rem:rem})); if(v===null) return;
  var arg = v.trim()===''?'full':v.trim();
  gas('carrierInvoicePayment',[id, arg]).then(function(r){ if(r&&r.ok){ toast(t('am.payRecorded'),'ok'); loadCarrierAp(); loadCarriers(); } else toast((r&&r.err)||'Hiba','err'); });
}
function carrierInvoiceDeleteUi(id){ if(!confirm(T('cs.confirmDeleteInvoice'))) return; gas('carrierInvoiceDelete',[id]).then(function(r){ if(r&&r.ok){ toast(t('am.deleted'),'ok'); loadCarrierAp(); loadCarriers(); } }); }
  ...
  gas('mapsGetProvider').then(function(r){
    ...
    if(vendor!=='free'){
      ...
      usageHtml='<div class="glass-soft" style="padding:12px 14px;margin-top:14px;">'
        ...
        +'<span style="font-size:12px;color:var(--muted);font-weight:700;">'+esc(T('cs.mpUsageMonth'))+' ('+vendor.toUpperCase()+')</span>'
        +'<span style="font-size:18px;font-weight:800;">'+m.toLocaleString('hu')+' <span style="font-size:12px;color:var(--muted);font-weight:500;">'+esc(T('cs.mpCalls'))+'</span></span>'
        +'<span style="margin-left:auto;font-size:12px;color:var(--muted);">'+esc(T('cs.mpToPay'))+'</span>'
        +'<span style="font-size:22px;font-weight:800;color:#fbbf24;">'+costM.toFixed(2)+' €</span></div>'
        +(costP?'<div style="font-size:11px;color:var(--muted);margin-top:4px;">'+esc(T('cs.mpPrevMonth'))+(usage.prev||0).toLocaleString('hu')+esc(T('cs.mpCallsWord'))+costP.toFixed(2)+' €</div>':'')
        +'<div style="font-size:10.5px;color:var(--muted);margin-top:6px;">⚠️ '+T('cs.mpNoFreeQuota',{pct:marginPct})+'</div>'
        +'</div>';
    }
    box.innerHTML='<div class="glass" style="padding:18px 20px;border:1px solid rgba(59,130,246,.35);">'
      +'<h3 style="font-size:16px;margin:0 0 4px;">'+esc(T('cs.mpTitle'))+'</h3>'
      +'<p style="color:var(--muted);font-size:12.5px;margin:0 0 14px;line-height:1.5;">'+T('cs.mpDesc')+'</p>'
      +'<label style="display:flex;align-items:center;gap:10px;cursor:pointer;font-size:13.5px;font-weight:600;margin-bottom:6px;">'
      +'<input type="checkbox" id="mpFree" '+(vendor==='free'?'checked':'')+' onchange="mpToggleFree()" style="width:18px;height:18px;cursor:pointer;accent-color:#22c55e;"> '+esc(T('cs.mpUseFree'))+'</label>'
      ...
      +'<div class="field" style="margin:0;min-width:170px;"><label>'+esc(T('cs.mpPaidProvider'))+'</label><select class="select" id="mpVendor">'
      +'<option value="here"'+(vendor==='here'?' selected':'')+'>'+esc(T('cs.mpHereRec'))+'</option>'
      +'<option value="google"'+(vendor==='google'?' selected':'')+'>'+esc(T('cs.mpGoogle'))+'</option></select></div>'
      +'<div class="field" style="margin:0;flex:1;min-width:240px;"><label>'+esc(T('cs.mpApiKey'))+(hasKey?'<span class="badge ok" style="font-size:10px;">'+esc(T('cs.mpStored'))+'</span>':'')+'</label><input class="input" id="mpKey" type="password" placeholder="'+(hasKey?esc(T('cs.mpKeyUnchanged')):esc(T('cs.mpProviderKey')))+'"></div>'
      +'<button class="btn ghost" style="height:42px;" onclick="mpTest()">'+esc(T('cs.mpTest'))+'</button>'
      +'</div><div id="mpMsg" style="margin-top:8px;font-size:12px;"></div>'+usageHtml+'</div>'
      +'<button class="btn primary" style="margin-top:14px;" onclick="mpSave()">'+esc(T('cs.save'))+'</button>'
      +'</div>';
  });
}
...
function mpTest(){
  ...
  var m=document.getElementById('mpMsg'); if(m) m.innerHTML='<span class="text-muted">'+esc(T('cs.mpTesting'))+'</span>';
  gas('mapsTestProvider',[{vendor:vendor, key:key}]).then(function(r){
    if(!m) return;
    if(r&&r.ok&&r.valid) m.innerHTML='<span style="color:var(--status-ok);">'+esc(T('cs.mpKeyWorks'))+'</span>';
    else if(r&&r.ok&&!r.valid) m.innerHTML='<span style="color:var(--status-danger);">'+esc(T('cs.mpKeyInvalid'))+'</span>';
    else m.innerHTML='<span style="color:var(--status-danger);">'+esc((r&&r.err)||T('cs.mpTestErr'))+'</span>';
  });
}
function mpSave(){
  ...
  gas('mapsSaveProvider',[{vendor:vendor, key:key}]).then(function(r){
    if(r&&r.ok){ toast(free?T('cs.mpFreeOn'):T('cs.mpSaved'),'ok'); loadMapsProvider(); }
    else toast((r&&r.err)||'Hiba','err');
  });
}
...
function loadInvites(){
  gas('invListAll').then(list=>{
    if(!Array.isArray(list)){document.querySelector('#tblInv tbody').innerHTML='<tr><td colspan="7" style="text-align:center;color:var(--muted);">'+esc(T('cs.noInviteCode'))+'</td></tr>';return;}
    document.querySelector('#tblInv tbody').innerHTML=list.map(i=>{
      var sc=i.status==='Aktiv'?'ok':i.status==='Felhasznalva'?'info':'err';
      var stLabel=i.status==='Aktiv'?T('cs.invAktiv'):i.status==='Felhasznalva'?T('cs.invUsed'):i.status;
      return '<tr>'
        ...
        +'<td>'+esc(i.email||'—')+'</td>'
        +'<td>'+esc(i.tel||'—')+'</td>'
        +'<td><span class="badge '+sc+'">'+esc(stLabel)+'</span></td>'
        +'<td><button class="btn ghost" style="padding:3px 10px;font-size:12px;" onclick="revokeInv(\''+esc(i.kod)+'\')" '+(i.status!=='Aktiv'?'disabled':'')+'>'+esc(T('cs.revoke'))+'</button></td>'
        +'</tr>';
    }).join('');
  }).catch(function(e){ console.error('loadInvites hiba:', e); toast(t('am.loadErr'),'err'); });
}
...
function loadReceivedFuvarlevelek(){
  gas('getFuvarlevelek').then(list=>{
    ...
    if(!list||list.length===0){tb.innerHTML='<tr><td colspan="5">'+esc(T('cs.noWaybillReceived'))+'</td></tr>';return;}
    tb.innerHTML=list.map(f=>`<tr>`
      ...
      +`<td style="display:flex;gap:6px;flex-wrap:wrap;">`
        +`<button class="btn ghost" style="padding:5px 10px;" onclick="openPdfView('${f.id}')">${esc(T('cs.viewPdf'))}</button>`
        +`<button class="btn primary" style="padding:5px 10px;" onclick="openFuvEdit('${f.id}')">${esc(T('cs.editPdf'))}</button>`
      +`</td></tr>`).join('');
  });
}
...
function printPdfView(){
  ...
  catch(e){ toast(t('am.printErr'),'err'); }
}
...
function openFuvEdit(id){
  gas('getFuvarlevelDetail',[id]).then(function(r){
    ...
    document.getElementById('feSeria').textContent=f.numar_fisa||T('cs.noSeqNumber');
...
function saveFuvEdit(){
  ...
  gas('fuvarlevelUpdate',[id,payload]).then(function(r){
    if(r&&r.ok){toast(t('am.waybillSaved'),'ok');closeFuvEdit();loadReceivedFuvarlevelek();}
...
function loadVehicles(){
  ...
  }).catch(function(e){ console.error('loadVehicles hiba:', e); toast(t('am.loadErr'),'err'); });
}
...
function orderFormPairFromVehicle(){
  ...
  if(remSel&&!remSel.value&&v.default_trailer_id){
    ...
    if(t){const ropt=[...remSel.options].find(o=>o.value===t.rendszam);
      if(ropt){remSel.value=t.rendszam;toast(t('am.pairedTrailer')+t.rendszam+T('cs.modifiable'),'ok');}}
  }
  ...
  toast(t('am.pairedDriver')+opt.text.split(' (')[0]+T('cs.modifiable'),'ok');
}
function orderFormPairFromDriver(){
  ...
  toast(t('am.pairedVehicle')+v.rendszam+T('cs.modifiable'),'ok');
}
...
function openChatRoom(roomId){
  ...
  if(hs)hs.textContent=meta.lastFrom?T('cs.chatLast')+meta.lastFrom+'':(meta.participants?meta.participants.join(', '):'');
  if(ha)ha.textContent=(label.replace(/[^a-zA-Z\u00C0-\u024F]/g,'')||'?').charAt(0).toUpperCase();

  var msgsEl=document.getElementById('chatMsgs');
  if(msgsEl)msgsEl.innerHTML='<div style="text-align:center;color:var(--muted);font-size:12px;padding:20px;">'+esc(T('cs.loadingDots'))+'</div>';
...
function quickStatusChange(id, sel) {
  ...
  .then(function(d){
    if (d.ok) {
      toast(t('am.statusOk') + newStatus, 'ok');
      var idx = -1;
  ...
  }).catch(function(){ toast(t('am.connErr'), 'err'); loadOrders(); });
}
...
function renderAdminRoomList(me){
  ...
  if(!myRooms.length){
    listEl.innerHTML='<div style="padding:16px 12px;font-size:12px;color:var(--muted);">'+T('cs.chatNoChat')+'</div>';
    return;
  }
...
function renderDocGroups() {
  ...
  if (!list.length) { container.innerHTML='<div style="text-align:center;color:var(--muted);padding:40px;">'+esc(T('cs.noResult'))+'</div>'; return; }
  var groups = {};
  ...
  Object.values(groups).forEach(function(g) {
    ...
    html += '<span style="font-size:12px;color:var(--muted);">'+g.docs.length+esc(T('cs.filesWord'))+'</span>';
    html += '</div>';
    ...
    g.docs.forEach(function(d) {
      ...
      html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap;"><span class="badge '+bc+'">'+esc(d.tip||T('cs.docOther'))+'</span>'
        +(d.order_id?'<span class="badge info" title="'+esc(T('cs.linkedToOrder'))+'">🔗 '+esc(d.order_id)+'</span>':'')
      ...
      html += '<a href="/api/doc-download/'+d.id+'" target="_blank" class="btn primary" style="flex:1;text-align:center;text-decoration:none;padding:8px 6px;font-size:12px;">'+esc(T('cs.docView'))+'</a>';
      html += '<a href="/api/doc-download/'+d.id+'" download class="btn ghost" style="flex:1;text-align:center;text-decoration:none;padding:8px 6px;font-size:12px;">'+esc(T('cs.docDownload'))+'</a>';
      html += '</div></div>';
    });
    html += '</div></div>';
  });
...
function revokeInv(kod){
  if(!confirm(T('cs.confirmRevoke',{kod:kod})))return;
  gas('invRevoke',[kod]).then(r=>{if(r.ok){toast(t('am.revoked'),'ok');loadInvites();}else{toast(r.err||'Hiba','err');}});
}

function saveAdminSigDraw(){var dataUrl=sigCanvas.toDataURL('image/png');gas('stampSave',[dataUrl]).then(()=>{toast(t('am.sigSaved'),'ok');loadAdminSigPreview();});}

function saveAdminSigFile(){var fi=document.getElementById('sigFile');if(!fi.files.length)return;var fr=new FileReader();fr.onload=function(e){gas('stampSave',[e.target.result]).then(()=>{toast(t('am.stampSaved'),'ok');loadAdminSigPreview();});};fr.readAsDataURL(fi.files[0]);}

function saveDocSeries() {
  var prefix = ((document.getElementById('docSeriesPrefix')||{}).value||'').trim();
  if (!prefix) { toast(t('am.givePrefix'),'err'); return; }
  fetch('/api/document-series',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({docType:'MT',prefix:prefix})})
  .then(function(r){return r.json();}).then(function(d){
    if (d.ok) { toast(t('am.seriesSaved')+prefix+'-'+new Date().getFullYear()+'-0001'+T('cs.seriesFrom'),'ok'); loadDocSeries(); }
...
function saveExtDriver(){
  ...
  if(!fields.nume&&!fields.firma){toast(t('am.driverOrCompanyReq'),'err');return;}
  gas('extDriverUpdate',[id,fields]).then(r=>{if(r.ok){toast(t('am.saved'),'ok');closeExtDriverModal();loadExtDrivers();}else{toast(r.err||'Hiba történt','err');}});
}
...
function saveQuickVehicle() {
  ...
  if (!v.rendszam) {
    toast(t('am.plateReq'), 'err');
    return;
  }
  gas('vehicleCreate', [v]).then(r => {
    if (r.ok) {
      toast((tip === 'Vontato' ? T('cs.tractorWord') : T('cs.trailerWord')) + T('cs.addedSuffix'), 'ok');
      closeQuickVehicle();
...
function saveVehicle(){
  ...
  if(!fields.rendszam){toast(t('am.plateReq'),'err');return;}
  gas('vehicleUpdate',[id,fields]).then(r=>{if(r.ok){toast(t('am.saved'),'ok');closeVehicleModal();loadVehicles();}else{toast(r.err||'Hiba történt','err');}});
}

function settings2faConfirm() {
  var code = document.getElementById('setup2faCode').value.trim();
  if (!code || code.length !== 6) { toast(t('am.give6Code'),'err'); return; }
  ...
  fetch('/api/2fa/settings-verify', {
    ...
    btn.disabled = false; btn.textContent = T('cs.confirm2fa');
    if (d.success) {
      document.getElementById('modal2faSetup').classList.remove('open');
      toast(t('am.twofaOn'),'ok');
      loadSettingsPane();
    } else {
...
function settings2faDisable(){
  ...
  if(!pwd){ toast(t('am.givePw'),'err'); return; }
  if(!confirm(T('cs.confirmDisable2fa'))) return;
  gas('settings2faDisable',[{currentPwd:pwd}]).then(function(r){
    if(r&&r.ok){
      toast(t('am.twofaOff'),'ok');
...
function settingsChangePassword(){
  ...
  if(!cur||!nw||!nw2){ toast(t('am.allFieldsReq'),'err'); return; }
  if(nw!==nw2){ toast(t('am.pw2Mismatch'),'err'); return; }
  if(nw.length<6){ toast(t('am.pwMin6'),'err'); return; }
  gas('settingsChangePassword',[{current:cur,newPwd:nw}]).then(function(r){
    if(r&&r.ok){
      toast(t('am.pwChanged'),'ok');
...
function settingsSaveProfile(){
  ...
  if(!nume){ toast(t('am.nameReq'),'err'); return; }
  gas('settingsSaveProfile',[{nume:nume,tel:tel}]).then(function(r){
    if(r&&r.ok){
      toast(t('am.profileSaved'),'ok');
...
function updateDocSeriesPreview() {
  ...
  if (el) el.textContent = T('cs.docSeriesPreview')+p+'-'+y+'-0001'+T('cs.docSeriesPreviewSuf');
}
...
function updateOrderSelBar() {
  ...
  if (checked.length > 0) {
    ...
    cnt.textContent = checked.length + T('cs.ordersSelected');
  } else {
...
function uploadOrderDoc(){
  ...
  if(!file){toast(t('am.pickPdf'),'err');return;}
  if(!currentDocOrderId){toast(t('am.noOrderSelected'),'err');return;}
  const reader=new FileReader();
  reader.onload=function(e){
    const b64=e.target.result;
    gas('orderDocUpload',[currentDocOrderId,file.name,b64]).then(r=>{
      if(r.ok){
        toast(t('am.uploadedX'),'ok');
        currentDocId=r.docId;
        loadDocList(currentDocOrderId);
        openSignModal(r.docId,'original');
      }else{
        toast(r.err||T('cs.uploadErr'),'err');
      }
    });
  };
...
function syncThemeToggleIcon() {
  ...
  btn.title = light ? T('cs.themeDark') : T('cs.themeLight');
}
  ...
  fetch('/api/invoices/summary?order_ids='+encodeURIComponent(ids.join(',')),{credentials:'same-origin'})
    .then(function(r){return r.json();}).then(function(d){
      ...
      document.querySelectorAll('[data-inv-ind]').forEach(function(el){
        ...
        if(s.stornoed){ el.textContent=' ↩️'; var t=T('cs.invStornoed')+(s.storno_serie||'')+'-'+(s.storno_numar||''); el.title=t; if(btn)btn.title=t; }
        else {
          ...
          var t2=T('cs.invInvoiced')+(s.serie||'')+'-'+(s.numar||'')+(s.efactura?(T('cs.efacturaLbl')+s.efactura):'');
  ...
  fetch('/api/uit/summary?order_ids='+encodeURIComponent(ids.join(',')),{credentials:'same-origin'})
    .then(function(r){return r.json();}).then(function(d){
      ...
      document.querySelectorAll('[data-uit-ind]').forEach(function(el){
        ...
        el.title=s.total+' UIT · '+T('cs.uitActive')+s.active+' '+T('cs.uitError')+s.error;
      });
    }).catch(function(){});
}
...
function addSignatureToPage(){
  ...
  if(signCanvasEl.toDataURL()===blank.toDataURL()){ toast(t('am.drawSigFirst'),'err'); return; }
  createDraggableItem(signCanvasEl.toDataURL('image/png'),'sign');
  toast(t('am.sigAdded'),'ok');
}

async function buildSignedPdf(){
  if(placedItems.length===0){ toast(t('am.noSigOrStamp'),'err'); return null; }
...
function downloadDoc(docId,which){
  gas('orderDocGet',[docId,which]).then(r=>{
    if(!r.ok||!r.base64){toast(r.err||T('cs.downloadErr'),'err');return;}
    let ext='.pdf';
...
function loadSettingsPane(){
  ...
  gas('settings2faStatus').then(function(r){
    ...
    if(r && r.totp_enabled){
      ...
      label.textContent      = T('cs.twofaActive');
      desc.textContent       = T('cs.twofaActiveDesc');
      disW.style.display     = '';
      enW.style.display      = 'none';
    } else {
      dot.style.background   = 'var(--warn)';
      label.textContent      = T('cs.twofaOff');
      desc.textContent       = T('cs.twofaOffDesc');
...
function openSignModal(docId,which){
  ...
  document.getElementById('signPageInfo').textContent=T('cs.loadingDots');
  ...
  gas('orderDocGet',[docId,which]).then(async r=>{
    if(!r.ok||!r.base64){ toast(r.err||T('cs.pdfLoadErr'),'err'); return; }
    try{
...
function downloadSelectedOrders() {
  var checked = document.querySelectorAll('.orderRowCb:checked');
  if (!checked.length) { toast(t('am.pickOneOrder'), 'err'); return; }
...
function openOrderEdit(id) {
  ...
  Promise.all([
    ...
    .then(r=>r.json()).then(function(d) {
      var o = d.result;
      var legs = d.legs || [];
      if (!o) { toast(t('am.notFound'),'err'); return; }
      ...
      camSel.onchange = function(){
        ...
        oeToggleSoferType();
        sel.value = u.email;
        toast(t('am.pairedDriver')+u.nume+T('cs.modifiable'),'ok');
      };
...
function renderOeLegs(legs) {
  ...
  if (!legs.length) {
    el.innerHTML = '<div style="font-size:12px;color:var(--muted);padding:8px;">'+esc(T('cs.noSwap'))+'</div>';
    return;
  }
  el.innerHTML = legs.map(function(leg) {
    return '<div style="background:var(--bg-2);border:1px solid var(--border);border-radius:10px;padding:10px 12px;font-size:13px;display:flex;justify-content:space-between;align-items:center;">'
      + '<div>'
      + '<b style="color:#fff;">' + leg.leg_number + T('cs.legSection') + '</b>'
...
function saveOrderEdit() {
  ...
  if(payload.load_type==='LTL' && (!payload.hossz_cm||!payload.szel_cm||!payload.mag_cm)){toast(t('am.ltlDimsReq'),'err');return;}

  gas('comUpdate', [_oeOrderId, payload]).then(function(r) {
    if (r && r.ok) {
      toast(t('am.savedOk'), 'ok');
      closeOrderEditModal();
      loadOrders();
    } else { toast(r&&r.err||'Szerver hiba','err'); }
  });
...
function savePayment(){
  ...
  if(!isFinite(amount)||amount<=0){ toast(t('am.giveValidAmount'),'err'); return; }
  var p={
    amount:amount,
  ...
  gas('markOrderPayment',[_payOrderId,p]).then(function(r){
    if(r&&r.ok){
      toast(r.payment_status==='paid'?T('cs.fullyPaid'):T('cs.partialRecorded'),'ok');
      closePaymentModal();
...
function resetPayment(){
  if(!confirm(T('cs.confirmResetPay'))) return;
  gas('markOrderPayment',[_payOrderId,{reset:true}]).then(function(r){
    if(r&&r.ok){ toast(t('am.payReset'),'ok'); closePaymentModal(); _afterPaymentRefresh(); }
    else toast((r&&r.err)||'Hiba','err');
  });
}
...
function copyTrackingLink(orderId){
  gas('getTrackingLink',[orderId]).then(function(r){
    if(!r||!r.ok){ toast((r&&r.err)||T('cs.linkGenErr'),'err'); return; }
    var url=location.origin+'/t/'+r.token;
    function done(){ toast(t('am.trackLinkCopied'),'ok'); }
    if(navigator.clipboard&&navigator.clipboard.writeText){
      navigator.clipboard.writeText(url).then(done).catch(function(){ prompt(T('cs.copyLink'),url); });
    } else { prompt(T('cs.copyLink'),url); }
  });
}
...
function ensureHandoverModal(){
  ...
  d.innerHTML=
    ...
    '<h3 id="hoTitle">'+esc(T('cs.handoverGiveTitle'))+'</h3>'+
    '<div class="text-muted" id="hoSub" style="font-size:12.5px;margin-bottom:12px;"></div>'+
    '<div class="field"><label>'+esc(T('cs.whatHappens'))+'</label>'+
      '<div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:4px;">'+
      '<label style="display:flex;align-items:center;gap:7px;cursor:pointer;"><input type="radio" name="hoType" value="trailer" checked onchange="hoTypeChange()"> '+esc(T('cs.parkTrailer'))+'</label>'+
      '<label style="display:flex;align-items:center;gap:7px;cursor:pointer;"><input type="radio" name="hoType" value="warehouse" onchange="hoTypeChange()"> '+esc(T('cs.goesWarehouse'))+'</label>'+
      '</div></div>'+
    '<div class="field"><label>'+esc(T('cs.whereCargo'))+'</label><input class="input" id="hoLoc" placeholder="pl. Brașov"></div>'+
    '<div class="field"><label>'+esc(T('cs.destChanges'))+'</label><input class="input" id="hoNewDest" placeholder="'+esc(T('cs.destChangesPh'))+'"></div>'+
    '<div id="hoWhBlock" style="display:none;">'+
      '<div class="text-muted" style="font-size:12px;font-weight:700;margin:6px 0 8px;">'+esc(T('cs.whDataReq'))+'</div>'+
      '<div class="grid-2">'+
      '<div class="field"><label>'+esc(T('cs.qty'))+'</label><input class="input" id="hoQty" type="number" min="1" placeholder="pl. 33"></div>'+
      '<div class="field"><label>'+esc(T('cs.unit'))+'</label><select class="select" id="hoQtyUnit"><option value="paletta">'+esc(T('cs.unitPallet'))+'</option><option value="doboz">'+esc(T('cs.unitBox'))+'</option><option value="egyeb">'+esc(T('cs.unitOther'))+'</option></select></div>'+
      '</div>'+
      '<div class="grid-3">'+
      '<div class="field"><label>'+esc(T('cs.lengthCm'))+'</label><input class="input" id="hoLen" type="number" min="1" placeholder="pl. 800"></div>'+
      '<div class="field"><label>'+esc(T('cs.widthCm'))+'</label><input class="input" id="hoWid" type="number" min="1" placeholder="pl. 248"></div>'+
      '<div class="field"><label>'+esc(T('cs.heightCm'))+'</label><input class="input" id="hoHei" type="number" min="1" placeholder="pl. 220"></div>'+
      '</div>'+
      '<div class="grid-2">'+
      '<div class="field"><label>'+esc(T('cs.weightKg'))+'</label><input class="input" id="hoWeight" type="number" min="1" step="0.01" placeholder="pl. 12500"></div>'+
      '<div class="field"><label>'+esc(T('cs.docPages'))+'</label><input class="input" id="hoDocPages" type="number" min="1" placeholder="pl. 10"></div>'+
      '</div>'+
      '<div class="text-muted" style="font-size:11.5px;margin-bottom:8px;">'+esc(T('cs.whUploadHint'))+'</div>'+
    '</div>'+
    '<div class="field"><label>'+esc(T('cs.note'))+'</label><input class="input" id="hoNote" placeholder="'+esc(T('cs.optional'))+'"></div>'+
    '<div class="row" style="margin-top:14px;display:flex;gap:10px;">'+
    '<button class="btn ghost col" onclick="closeHandoverModal()">'+esc(T('cs.cancel'))+'</button>'+
    '<button class="btn primary col" id="hoSubmitBtn" onclick="submitHandover()">'+esc(T('cs.saveHandover'))+'</button>'+
    '</div></div>';
  document.body.appendChild(d);
}
...
function openHandoverModal(orderId, prefill, mode){
  ...
  var p=prefill||{};
  document.getElementById('hoTitle').textContent = _hoMode==='confirm' ? T('cs.handoverConfirmTitle') : T('cs.handoverGiveTitle');
  document.getElementById('hoSub').textContent = T('cs.orderColon')+orderId+
    (_hoMode==='confirm' ? T('cs.handoverConfirmSub') : T('cs.handoverDirectSub'));
  document.getElementById('hoSubmitBtn').textContent = _hoMode==='confirm' ? T('cs.confirmHandover') : T('cs.saveHandover');
...
function submitHandover(){
  ...
  if(!data.location){toast(t('am.handoverPlaceReq'),'err');return;}
  if(type==='warehouse'){
  ...
  gas(fn,[oid,data]).then(function(r){
    if(r&&r.ok){
      toast(type==='trailer'?T('cs.cargoParked'):T('cs.cargoWarehoused'),'ok');
      closeHandoverModal();
      if(typeof loadOrders==='function')loadOrders();
...
function loadWarehouseTab(){
  ...
  if(!box)return;
  box.innerHTML='<div class="glass" style="padding:20px;color:var(--text-muted);">'+esc(T('cs.loadingDots'))+'</div>';
  gas('getWarehouseItems').then(function(list){
    ...
    var html='<div class="glass" style="padding:22px;">'+
      '<h2 class="h-title">'+esc(T('cs.whTitle'))+'</h2>'+
      '<div class="h-sub">'+esc(T('cs.whSub'))+'<b>'+act.length+'</b>'+
      (noDocs?' · <span style="color:var(--status-danger);font-weight:700;">⚠️ '+noDocs+esc(T('cs.whMissingDoc'))+'</span>':'')+'</div></div>';
    if(!list.length){
      html+='<div class="glass text-muted" style="padding:22px;margin-top:16px;">'+esc(T('cs.whEmpty'))+'</div>';
      box.innerHTML=html; return;
    }
    html+='<div class="glass" style="padding:22px;margin-top:16px;overflow-x:auto;">'+
      '<table class="table"><thead><tr><th>'+esc(T('cs.whColOrder'))+'</th><th>'+esc(T('cs.whColClient'))+'</th><th>'+esc(T('cs.whColLocation'))+'</th><th>'+esc(T('cs.whColQty'))+'</th>'+
      '<th>'+esc(T('cs.whColSpace'))+'</th><th>'+esc(T('cs.whColWeight'))+'</th><th>'+esc(T('cs.whColDocPages'))+'</th><th>'+esc(T('cs.whColDocs'))+'</th><th>'+esc(T('cs.whColStored'))+'</th><th>'+esc(T('cs.whColStatus'))+'</th><th>'+esc(T('cs.whColAction'))+'</th></tr></thead><tbody>'+
      list.map(function(w){
        var docs=parseInt(w.doc_count,10)||0;
        var inWh=w.status==='Raktarban';
        var docCell=docs>0
          ?'<span style="color:var(--status-ok);">📷 '+docs+esc(T('cs.whDocsCount'))+'</span>'
          :(inWh?'<span class="badge err">'+esc(T('cs.whDocMissing'))+'</span>':'<span class="text-muted">—</span>');
        return '<tr'+(inWh&&!docs?' style="background:rgba(239,68,68,0.06);"':'')+'>'+
          '<td><b>'+esc(String(w.order_id))+'</b></td>'+
          '<td>'+esc(w.client||'—')+'</td>'+
          '<td>'+esc(w.location||'—')+'<div class="text-muted" style="font-size:11px;">'+esc(T('cs.whFinalDest'))+esc(w.loc_descarcare||'—')+'</div></td>'+
          ...
          '<td><span class="badge '+(inWh?'warn':'info')+'">'+(inWh?T('cs.whStatusIn'):T('cs.whStatusOut'))+'</span></td>'+
          '<td style="display:flex;gap:4px;flex-wrap:wrap;">'+
            '<button class="btn ghost" style="padding:4px 10px;font-size:12px;" title="'+esc(T('cs.whUploadDoc'))+'" onclick="openDocModal(\''+esc(String(w.order_id))+'\')">📎</button>'+
            '<button class="btn primary" style="padding:4px 10px;font-size:12px;" title="'+esc(T('cs.whEditOrder'))+'" onclick="openOrderEdit(\''+esc(String(w.order_id))+'\')">✏️</button>'+
          '</td></tr>';
...
// ── Nyelvváltáskor a dinamikusan renderelt listák újrarajzolása ──
// A statikus DOM-ot az i18n.js intézi; itt csak a JS-ből renderelt
// felületeket frissítjük. Minden hívás GUARDOLT: ha a konténer nincs
// jelen (más fül aktív), no-op. A status-/label-térképek render-időben
// fordulnak (a kulcsot tárolják), így az újrarender mindig friss nyelvű.
if(window.I18N && window.I18N.onLang){
  window.I18N.onLang(function(){
    try{
      // Fuvarlista (cache-ből, ha van) + függő leadás-banner
      if(typeof renderFilteredOrders==='function' && window._ordersAllCache) renderFilteredOrders(window._ordersAllCache);
      if(typeof loadPendingHandovers==='function' && document.querySelector('.pane[data-pane="orders-list"]')) loadPendingHandovers();
      // Vezérlőpult (csak ha látható)
      var dashPane=document.querySelector('.pane[data-pane="dash"]');
      if(dashPane && !dashPane.classList.contains('hidden') && typeof loadDashboard==='function') loadDashboard();
      // Az áru-leadás modal egyszer épül — nyelvváltáskor építsük újra
      var hm=document.getElementById('handoverModal');
      if(hm && !hm.classList.contains('open')){ hm.remove(); }
      // A többi fül-tartalmat csak akkor frissítjük, ha a konténere jelen van
      if(typeof loadWarehouseTab==='function' && document.getElementById('warehouseBox')) loadWarehouseTab();
      if(typeof loadCarriers==='function' && document.getElementById('carriersBox')) loadCarriers();
      if(typeof loadCarrierAp==='function' && document.getElementById('carrierApBox')) loadCarrierAp();
      if(typeof loadClientPortalAccess==='function' && document.getElementById('clientPortalAccessBox')) loadClientPortalAccess();
      if(typeof loadMapsProvider==='function' && document.getElementById('mapsProviderBox')) loadMapsProvider();
      if(typeof loadInvites==='function' && document.querySelector('#tblInv tbody')) loadInvites();
      if(typeof loadInternalDrivers==='function' && document.querySelector('#tblInternalDrivers tbody')) loadInternalDrivers();
      if(typeof loadExtDrivers==='function' && document.querySelector('#tblExtDrivers tbody')) loadExtDrivers();
      if(typeof loadVehicles==='function' && (document.querySelector('#tblVontato tbody')||document.querySelector('#tblPotkocsi tbody'))) loadVehicles();
      if(typeof loadReceivedFuvarlevelek==='function' && document.querySelector('#tblReceivedFuv tbody')) loadReceivedFuvarlevelek();
      if(typeof renderDocGroups==='function' && document.getElementById('docGroupsContainer')) renderDocGroups();
      var setPane=document.querySelector('.pane[data-pane="settings"]');
      if(setPane && !setPane.classList.contains('hidden') && typeof loadSettingsPane==='function') loadSettingsPane();
    }catch(e){ /* nyelvváltás sosem buktathat fel UI-t */ }
  });
}
