  ...
  const settled = await Promise.allSettled(
    [...byObjectId.values()].map(async (m) => {
      ...
      return {
        ...
        // Élő üzemanyag-szint (a CargoTrack calculated_inputs.fuel_level-ből),
        // ha az eszköz méri — ezt jelenítjük meg a térkép-tooltipban.
        fuel_level: (st.fuel_level != null && isFinite(parseFloat(st.fuel_level))) ? parseFloat(st.fuel_level) : null,
      };
    })
  );
...
async function getReadingsForPlate(cid, plate) {
  ...
  const battery_voltage = (st.battery_voltage != null && isFinite(parseFloat(st.battery_voltage))) ? parseFloat(st.battery_voltage) : null;
  const state_of_charge = (st.state_of_charge != null && isFinite(parseFloat(st.state_of_charge))) ? parseFloat(st.state_of_charge) : null;
  if (mileage == null && fuel_level == null && battery_voltage == null && state_of_charge == null) return { ok: true, available: false };
  return { ok: true, available: true, mileage, fuel_level, battery_voltage, state_of_charge, datetime: st.datetime || null };
}
