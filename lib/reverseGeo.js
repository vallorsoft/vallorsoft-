...
// Timeout 6 mp. Nincs cache — a menetlevél-Plecare kb. napi 1× hívja, alacsony
// forgalom mellett a rate-limit is elég laza.
...
// null = mindkét provider hibázott / üres.
async function reverseGeocode(lat, lng, lang) {
  ...
  try {
    ...
    if (p) return { address: p };
  } catch (_) {}
  try {
    const n = await _nominatimReverse(la, ln, lang);
    if (n) return { address: n };
  } catch (_) {}
  return null;
}
