...
async function getConfig(companyId) {
  ...
  let cfg = { vendor: 'free', key: null, reason: 'no-row' };
  try {
    ...
    if (!r.rows.length) {
      ...
      let key = null;
      let decryptOk = true;
      try { key = r.rows[0].credentials_enc ? decrypt(r.rows[0].credentials_enc) : null; }
      catch (e) { key = null; decryptOk = false; }
      if (vendor === 'free') cfg.reason = 'vendor-free';
      else if (!r.rows[0].credentials_enc) cfg.reason = 'no-credentials';
      else if (!decryptOk) cfg.reason = 'decrypt-failed';
      else if (!key) cfg.reason = 'empty-key';
      else cfg = { vendor, key, reason: 'ok' };
    }
  } catch (e) { cfg.reason = 'db-error:' + (e.message || 'unknown'); }
