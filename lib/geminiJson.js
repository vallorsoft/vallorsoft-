...
// A modell-lánc — CSAK STABIL, INGYENES napi kvótás flash modellek.
// Mindegyiknek KÜLÖN napi ingyenes kerete van, így a 429-es hibáknál
// (kvóta/perc-limit) a következő modellre váltunk, és a keretek
// gyakorlatilag összeadódnak.
//
// SZÁNDÉKOSAN KIMARADNAK a pro / ultra / experimental modellek:
//   - gemini-1.5-pro / -pro-002: fizetős tier, kis vagy 0 ingyenes keret.
//   - gemini-2.5-pro: van szabad tier, de mikroszkopikus (2 RPM / 50 RPD).
//   - gemini-*-exp / gemini-exp-1206: kísérleti/preview → gyakran 404-et
//     dobnak (visszavonva), ami a láncot megtörné.
// Ezek okozták az előző lánc (PR #287) hibáit: a 4 flash után 1.5-flash
// visszavonva → 404 → régi `extractJson` csak 429/503-on ugrott tovább,
// így 404-re ELHASALT (a maradék pro/exp modellek sose futottak). Az új
// `extractJson` a 404-et is toleránsan ugrik (defenzív), így visszavont
// modell nem tud beragadni a láncba.
...
async function extractJson({ systemPrompt, parts, models } = {}) {
  ...
  for (const model of list) {
    try {
      ...
      // FALLBACK-státuszok: ezeknél a következő modellre lépünk (mindegyiknek
      // külön napi kerete / szerver-példánya van, tehát érdemes próbálni).
      //   429 → kvóta/RPM-limit ezen a modellen
      //   503 → átmeneti túlterhelés (a callGemini már 3-szor retryzott 5xx-re)
      //   500 → belső hiba (a callGemini már retryzott — persistent)
      //   404 → a modell erre az API-kulcsra / régióra nem elérhető (retired
      //         vagy nincs engedélyezve). A régi lánc ezen ELHASALT (pl.
      //         `gemini-1.5-flash` 2025-09-24 után visszavonva) — most
      //         csendesen továbblép a következő modellre.
      if (e.status === 429 || e.status === 503 || e.status === 500 || e.status === 404) continue;
      // Egyéb (400 rossz kérés, 401 auth, 403 gate): azonnal áll — a
      // következő modell UGYANEZT dobná (a bemenet vagy az auth a hibás,
      // nem a modell), a lánc-továbblépés csak késleltetné a hiba-visszajelzést.
