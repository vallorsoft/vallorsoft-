...
function guard(req, res, next) {
  if (!req.session || !req.session.user) return res.status(401).send('Autentificare necesara.');
  if (!['Admin', 'Manager', 'Konyvelo'].includes(req.session.user.pozicio)) return res.status(403).send('Nu aveti permisiune.');
  next();
}
...
async function fetchDoc(cid, src, id) {
  if (src === 'order_doc') {
    ...
    return buf && { name: r.rows[0].file_name || 'document.pdf', buffer: buf, order_id: r.rows[0].order_id, created_at: r.rows[0].created_at };
  }
  if (src === 'pod') {
  ...
  if (src === 'carrier_doc') {
    ...
    return buf && { name: r.rows[0].file_name || 'incarcare', buffer: buf, order_id: r.rows[0].order_id, created_at: r.rows[0].created_at };
  }
  return null;
}
...
router.get('/api/accounting/doc', guard, async (req, res) => {
  try {
    ...
    if (!d) return res.status(404).send('Nu a fost gasit.');
  ...
  } catch (err) { console.error('accounting doc hiba:', err); return res.status(500).send('Eroare de server'); }
});
...
router.post('/api/accounting/zip', guard, async (req, res) => {
  try {
    ...
    if (!refs || !refs.length) {
      if (!b.from || !b.to) return res.status(400).send('Specificati o selectie sau o perioada.');
    ...
    for (const ref of refs) {
      ...
      const folder = (d.order_id ? String(d.order_id) : 'altele').replace(/[\\/:*?"<>|]/g, '_');
    ...
    if (!files.length) return res.status(404).send('Nu exista documente de descarcat in selectie.');
    const zip = buildZip(files);
    const fn = 'vallorsoft-documente-' + (b.from || dstr(Date.now())) + '.zip';
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', 'attachment; filename="' + fn + '"');
    return res.send(zip);
  } catch (err) {
    console.error('accounting zip hiba:', err);
    return res.status(500).send('Eroare de server la crearea ZIP-ului.');
  }
});
...
router.get('/api/accounting/invoices.csv', guard, async (req, res) => {
  try {
    ...
    if (kind === 'sales') {
      ...
      return sendCsv(res, 'facturi-emise.csv',
        ['Data', 'Numar', 'Partener', 'CUI', 'Suma', 'Moneda', 'TVA%', 'Furnizor', 'Stare', 'Cursa'], rows);
    }
    ...
    return sendCsv(res, 'facturi-primite.csv',
      ['Data', 'Scadenta', 'Numar factura', 'Subcontractor', 'CUI', 'Suma', 'Moneda', 'Stare', 'Platit', 'Curse'], rows);
  } catch (err) {
    console.error('accounting csv hiba:', err);
    return res.status(500).send('Eroare de server la crearea CSV-ului.');
  }
});
