...
router.post('/api/branding/logo', requireLogin, requireRole('Admin', 'Manager'), async (req, res) => {
  ...
  if (!b64) return res.status(400).json({ error: 'Hiányzik a kép.' });
  if (b64.length > 4 * 1024 * 1024) return res.status(413).json({ error: 'A logó túl nagy (max ~3 MB).' });
  try {
    await pool.query(
...
router.post('/api/email-templates', requireLogin, requireRole('Admin', 'Manager'), async (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'A sablon neve kötelező.' });
  try {
...
router.post('/api/client-mail/send', requireLogin, requireRole('Admin', 'Manager'), async (req, res) => {
  ...
  if (!to || !html) return res.status(400).json({ error: 'A címzett és az üzenet kötelező.' });
  try {
