...
function buildPaymentEmailHtml({ company, plan, billing, amountEur, amountRon, tvaRon, totalRon, bnrRate, reference, bankDetails, appUrl }) {
  ...
  const bankHtml = bankDetails
    ? `<table style="width:100%;border-collapse:collapse;margin:12px 0;">
        <tr><td style="padding:6px 0;color:#8a7d6e;font-size:13px;">Titular cont</td>
            <td style="padding:6px 0;font-weight:600;font-size:13px;">${escH(bankDetails.holder || '')}</td></tr>
        <tr><td style="padding:6px 0;color:#8a7d6e;font-size:13px;">IBAN</td>
            <td style="padding:6px 0;font-weight:600;font-size:13px;font-family:monospace;">${escH(bankDetails.iban || '')}</td></tr>
        <tr><td style="padding:6px 0;color:#8a7d6e;font-size:13px;">Bancă</td>
            <td style="padding:6px 0;font-weight:600;font-size:13px;">${escH(bankDetails.bank || '')}</td></tr>
        ${bankDetails.swift ? `<tr><td style="padding:6px 0;color:#8a7d6e;font-size:13px;">SWIFT/BIC</td>
  ...
  return `<div style="font-family:sans-serif;max-width:580px;margin:0 auto;color:#2a2018;">
  ...
  <div style="background:#faf6f0;padding:28px 32px;border:1px solid #ece3d8;border-top:none;border-radius:0 0 12px 12px;">
    ...
    <!-- Összesítő táblázat -->
    <div style="background:#fbf7f1;border:1px solid #ece3d8;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
      <div style="font-size:13px;color:#8a7d6e;margin-bottom:8px;font-weight:600;">REZUMAT PLATĂ</div>
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:4px 0;font-size:13px;color:#8a7d6e;">Pachet</td>
            <td style="padding:4px 0;font-size:13px;text-align:right;">${escH(plan.name)} (${perioadas})</td></tr>
        <tr><td style="padding:4px 0;font-size:13px;color:#8a7d6e;">Preț net EUR</td>
            <td style="padding:4px 0;font-size:13px;text-align:right;">€${eurStr}</td></tr>
        <tr><td style="padding:4px 0;font-size:13px;color:#8a7d6e;">Curs BNR EUR/RON</td>
            <td style="padding:4px 0;font-size:13px;text-align:right;">${rateStr} RON/€</td></tr>
        <tr><td style="padding:4px 0;font-size:13px;color:#8a7d6e;">Preț net RON</td>
            <td style="padding:4px 0;font-size:13px;text-align:right;">${ronStr} RON</td></tr>
        <tr><td style="padding:4px 0;font-size:13px;color:#8a7d6e;">TVA 21%</td>
            <td style="padding:4px 0;font-size:13px;text-align:right;">${tvaStr} RON</td></tr>
        <tr style="border-top:2px solid #f6711e;">
          <td style="padding:8px 0 4px;font-size:15px;font-weight:700;">TOTAL</td>
          <td style="padding:8px 0 4px;font-size:15px;font-weight:700;text-align:right;color:#f6711e;">${totalStr} RON</td>
        </tr>
      </table>
    </div>
    ...
    <!-- Referencia kód -->
    <div style="background:linear-gradient(135deg,rgba(246,113,30,.08),rgba(246,81,123,.08));border:2px solid #f6711e;border-radius:10px;padding:16px 20px;margin-bottom:20px;text-align:center;">
      <div style="font-size:12px;color:#f6711e;font-weight:700;letter-spacing:.5px;margin-bottom:6px;">REFERINȚĂ PLATĂ</div>
      <div style="font-size:26px;font-weight:800;font-family:monospace;letter-spacing:2px;color:#2a2018;">${escH(reference)}</div>
      <div style="font-size:12px;color:#8a7d6e;margin-top:6px;">Scrieți acest cod în câmpul „Observații" al ordinului de plată.</div>
    </div>

    <p style="font-size:13px;color:#8a7d6e;margin:0 0 20px;">
    ...
    <a href="${appUrl}/admin" style="display:inline-block;background:linear-gradient(180deg,#fb8c3a,#f6711e);color:#fff;text-decoration:none;padding:11px 22px;border-radius:8px;font-weight:600;font-size:14px;">
      Intră în aplicație
    </a>

    <p style="margin:20px 0 0;font-size:12px;color:#b09a82;">
      Întrebări? <a href="mailto:vallorsoft@gmail.com" style="color:#f6711e;">vallorsoft@gmail.com</a>
      · <a href="${appUrl}/terms" style="color:#f6711e;">Termeni</a>
    </p>
  </div>
</div>`;
}
...
<style>body{font-family:sans-serif;background:#faf6f0;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;}
.box{background:#fff;border-radius:16px;padding:40px;max-width:520px;text-align:center;box-shadow:0 4px 24px rgba(42,32,24,.08);border:1px solid #ece3d8;}
h1{font-size:22px;color:#2a2018;margin-bottom:8px;}
p{color:#8a7d6e;line-height:1.6;font-size:15px;}
.ref{font-size:28px;font-weight:800;font-family:monospace;color:#f6711e;letter-spacing:2px;background:rgba(246,113,30,.08);padding:10px 20px;border-radius:8px;display:inline-block;margin:12px 0;}
a.btn{display:inline-block;margin-top:20px;background:linear-gradient(180deg,#fb8c3a,#f6711e);color:#fff;text-decoration:none;padding:11px 24px;border-radius:8px;font-weight:600;}</style>
</head>
<body><div class="box">
